...
from module_list import libsage_modules, cython_modules
...
cython_modules = cython_modules + sage.ext.gen_interpreters.modules
...
# In the loops below, don't append to any list, since many of these
# lists are actually identical Python objects. For every list, we need
# to write (at least the first time):
#
#   list = list + [foo]
#
for m in libsage_modules + cython_modules:
...
class sage_build_ext(build_ext):
    ...
    def build_extensions(self):
        ...
        for ext in self.extensions:
            ...
            if need_to_compile:
                command = (record_compile(self.build_extension), p)
...
################################################################
### Installing libraries in local/lib instead of site-packages
################################################################

from distutils.command.build import build as build_base
from distutils.command.install_lib import install_lib as install_lib_base
class build_system_lib(build_base):
    def initialize_options(self):
        build_base.initialize_options(self)
        self.build_base = os.path.join('build', 'c_lib')

class install_system_lib(install_lib_base):
    def initialize_options(self):
        install_lib_base.initialize_options(self)
        self.install_dir = os.path.join(SAGE_LOCAL, 'lib')
...
for output_dir in output_dirs:
    ...
    clean_install_dir(output_dir, python_packages, python_modules, cython_modules)
...
# Build/install shared libraries (libcsage)
setup(
    cmdclass={ 'build': build_system_lib, 'build_ext': sage_build_ext, 'install_lib': install_system_lib },
    ext_modules=libsage_modules,
    **metadata)

# Build/install Cython extensions and Python packages
setup(
