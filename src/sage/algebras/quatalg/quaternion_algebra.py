...
class QuaternionAlgebra_ab(QuaternionAlgebra_abstract):
    ...
    def quaternion_lattice(self, basis, check=True):
        r"""
        Returns a lattice in the quaternion algebra generated by basis.
        
        INPUT:

        - ``basis`` - A basis of the lattice
        - ``check`` - if check=True, this checks if basis spans a 4 dimensional vector space.

        EXAMPLES::

            sage: B.<i, j, k>=QuaternionAlgebra(-1,-1)
            sage: L=B.quaternion_lattice([1, i + 2*j, j, k]); L
            Lattice in Quaternion Algebra (-1, -1) with base ring Rational Field with basis [1, i + 2*j, j, k]

        """
        return QuaternionLattice(self, basis, check=check)
...
class QuaternionLattice(ParentWithGens):

    def __init__(self, A, basis, check=True):
        r"""
        Create a Quaternion lattice class to be used for
        quaternion orders and ideals.

        INPUT:

            - ``A`` -- a quaternion algebra

            - ``basis`` -- a basis for the lattice given by
              elements of A

            - ``check`` -- if check=True, this checks if basis spans a 4 dimensional vector space.

        OUTPUT: A ZF lattice over the quaternions, where ZF is the
        maximal order of the number field the quaternion algebra is
        defined over.

        EXAMPLES::

            sage: A.<i, j, k>=QuaternionAlgebra(-1, -1)
            sage: L = A.quaternion_lattice([1, i + 2*j, j, k]); L
            Lattice in Quaternion Algebra (-1, -1) with base ring Rational Field with basis [1, i + 2*j, j, k]

            sage: F.<a> = NumberField(x^2 - x - 1)
            sage: ZF = F.maximal_order()
            sage: B.<i,j,k> = QuaternionAlgebra(F, -1, -1)
            sage: L = B.quaternion_lattice([1, i, j, k]); L
            Lattice in Quaternion Algebra (-1, -1) with base ring Number Field
            in a with defining polynomial x^2 - x - 1 with basis [1, i, j, k]
            sage: M = B.quaternion_lattice([a, a*i, a*j, a*k])
            sage: L.intersection(M)
            Lattice in Quaternion Algebra (-1, -1) with base ring Number Field
            in a with defining polynomial x^2 - x - 1 with basis [-a, (-a)*i, (-a)*j, (-a)*k]
            sage: M == L
            False

        The following example shows a bug (or lack of implementation)
        in comparisons.

        ::

            sage: M = B.quaternion_lattice([i, 1, j, k])
            sage: M == L
            False

        """

        #Coerce basis into the quaternion algebra.
        basis = [A(b) for b in basis]
        ...
        if check:
            ...
            # has rank 4
            V = A.base_ring()**4
            if V.span([x.coefficient_tuple() for x in basis]).dimension() != 4:
                raise ValueError("basis must have rank 4")
        ...
        ZF = F.maximal_order()
        self._base_ring = ZF
        self._free_module_basis = [v.coefficient_tuple() for v in self._basis]
        self._free_module = (ZF**4).span(self._free_module_basis, self._base_ring)
        # using A.base_ring()**4 instead of A.vector_space() because span doesn't work in the latter case
        # free_module should work fine over number fields after Aly's patch
        self._gens = basis
        #get the gram matrix from A
        self._gram_matrix = None
        ParentWithGens.__init__(self, self._base_ring)

    def __cmp__(self, other):
        r"""
        Compare the lattices self and other.  Two lattices are equal if their
        basis span the same module over the base ring.

        INPUT:

        - ``other`` -- a quaternion lattice

        OUTPUT: True if the lattice equals other, and False otherwise.

        EXAMPLES::

            sage: B.<i, j, k> = QuaternionAlgebra(-1, -1)
            sage: L = B.quaternion_lattice([1, i, j, k + i])
            sage: M = B.quaternion_lattice([1, i - j, j, k])
            sage: L == M
            True

        """
        return cmp(self._free_module, other.free_module())

    def __add__(self, other):
        r"""
        Returns the sum of the lattices self and other.

        INPUT:

        - ``other`` -- a quaternion lattice

        OUTPUT: the  sum of the quaternion lattice with other.

        EXAMPLES::

            sage: F.<r> = NumberField(x^3 - x - 1)
            sage: B.<i, j, k> = QuaternionAlgebra(2 + r, 5)
            sage: L1 = B.quaternion_lattice([1, i+j, 2*j, 2*k])
            sage: L2 = B.quaternion_lattice([1, i, -7*j, k+3*i])
            sage: L1 + L2
            Lattice in Quaternion Algebra (r + 2, 5) with base ring Number Field
            in r with defining polynomial x^3 - x - 1 with basis [1, i, j, k]
            ...
            in r with defining polynomial x^3 - x - 1 with basis [1, i, j, k]
        """
        B = (self._free_module + other.free_module()).basis()
        A = self.__quaternion_algebra
        Bas = [A(tuple(v)) for v in B]
        return QuaternionLattice(A, Bas)

    def is_sublattice(self, other):
        r"""
        Returns True if the lattice is included in the quaternion lattice other, and False if not.

        INPUT:

        - ``other`` -- a quaternion lattice

        OUTPUT: True if the quaternion lattice is a sublattice of other, and if not.

        EXAMPLES::

            sage: B.<i, j, k> = QuaternionAlgebra(15)
            sage: L1 = B.quaternion_lattice([1, i, 6*j, -12*k])
            sage: L2 = B.quaternion_lattice([1, i, j, 2*k+i])
            sage: L1.is_sublattice(L2)
            True

            sage: F.<r> = NumberField(x^3 - x - 1)
            sage: B.<i, j, k> = QuaternionAlgebra(2 + r, 5)
            sage: L1 = B.quaternion_lattice([1, i + j, 2*j, 2*k])
            sage: L2 = B.quaternion_lattice([1, 3*i, j, k])
            sage: L1.is_sublattice(L2)
            False
        """
        return self._free_module.is_submodule(other.free_module())

    def intersection(self, other):
        r"""
        Computes the intersection of quaternion lattice.

        INPUT:

        - ``other`` -- a quaternion lattice

        OUTPUT: the quaternion lattice which is the intersection of self and other.

        EXAMPLES::

            sage: B.<i,j,k> = QuaternionAlgebra(10)
            sage: L1 = B.quaternion_lattice([1, i, j, 2*k])
            sage: L2 = B.quaternion_lattice([1, i+3*j, k, 2*i])
            sage: L1.intersection(L2)
            Lattice in Quaternion Algebra (-2, 5) with base ring Rational Field with basis [1, i + 3*j, 6*j, 2*k]
        """
        B = self._free_module.intersection(other.free_module()).basis()
        A = self.__quaternion_algebra
        Bas = [A(tuple(v)) for v in B]
        return QuaternionLattice(A, Bas)
    ...
    def free_module(self):
        r"""
        Returns the underlying free module of the lattice.

        EXAMPLES::

            sage: B.<i,j,k> = QuaternionAlgebra(2, 3)
            sage: L = B.quaternion_lattice([1, i + j, 2*j, 2*k])
            sage: L.free_module()
            Free module of degree 4 and rank 4 over Integer Ring
            Echelon basis matrix:
            [1 0 0 0]
    ...
    def basis(self):
        r"""
        Returns the basis of the lattice over the base ring of the lattice.
    ...
    def base_ring(self):
        r"""
        Returns the base ring of the quaternion lattice, namely the ring of integers of the field over which A is defined.

        EXAMPLES::

            sage: B.<i, j, k> = QuaternionAlgebra(15)
            sage: L = B.quaternion_lattice([1, i, 6*j,-12*k])
            sage: L.base_ring()
            Integer Ring
        """
    ...
    def _repr_(self):
        ...
        EXAMPLES::
            ...
            sage: F.<r>=NumberField(x^3 - x - 1)
            sage: B.<i, j, k> = QuaternionAlgebra(2 + r, 5)
            sage: L=B.quaternion_lattice([1, i + j, 2*j, 2*k]); L
            Lattice in Quaternion Algebra (r + 2, 5) with base ring Number Field in r with defining
            polynomial x^3 - x - 1 with basis [1, i + j, 2*j, 2*k]

        """
        return 'Lattice in %s with basis %s'%(self.__quaternion_algebra, self._basis)


    def conjugate_lattice(self):
        r"""
        Returns the conjugate lattice of self
        ...
        EXAMPLES::
            ...
            sage: B.<i, j, k> = QuaternionAlgebra(2 + r, 5)
            sage: L = B.quaternion_lattice([1, i + j, 2*j, 2*k])
            sage: L.conjugate_lattice()
            Lattice in Quaternion Algebra (r + 2, 5) with base ring Number Field in r with defining
            polynomial x^3 - x - 1 with basis [1, -i - j, (-2)*j, (-2)*k]

        """
        conj_basis = [x.conjugate() for x in self._basis]
        return QuaternionLattice(self.__quaternion_algebra, conj_basis)

    def is_order(self):
        r"""
        Returns True if the self is an order, and False otherwise.

        EXAMPLES::

            sage: B.<i, j, k>=QuaternionAlgebra(-1, -7)
            sage: L = B.quaternion_lattice([1, i, (1+j) / 2, (i+k) / 2])
            sage: L.is_order()
            True

            sage: F.<r> = NumberField(x^3 - x - 1)
            sage: B.<i, j, k> = QuaternionAlgebra(2 + r, 5)
            sage: L = B.quaternion_lattice([1, i, j, k])
            sage: L.is_order()
            False

        .. WARNING::

            As the last example above illustrates the method is_order does not work corretly
            over general number fieds.

        """
        ZF = self._base_ring
        L = self._free_module
        V = self.__quaternion_algebra.vector_space()
        if not V((1, 0, 0, 0)) in L:
            return False
        import itertools
        for x, y in itertools.product(*[self._basis, self._basis]):
            if not V((x*y).coefficient_tuple()) in L:
                return False
        return True
