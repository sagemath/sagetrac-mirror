...
- Xavier Guitart and Marc Masdeu (2014-09): implemented constructor
  for number fields from ramified places.
...
class QuaternionAlgebraFactory(UniqueFactory):
    ...
    - ``QuaternionAlgebra(K, D, S)``: a quaternion algebra over a number
      field ``K``. Here, ``D`` is the discriminant and ``S`` is the set of
      infinite places where the quaternion algebra ramifies.
    ...
    *smallest* field containing the nonzero elements ``a`` and ``b`` with
    ...
    def create_key(self, arg0, arg1=None, arg2=None, names='i,j,k'):
        ...
        # QuaternionAlgebra(K, a, b) or QuaternionAlgebra(K, D, S)
        else:
            K = arg0
            if K not in _Fields:
