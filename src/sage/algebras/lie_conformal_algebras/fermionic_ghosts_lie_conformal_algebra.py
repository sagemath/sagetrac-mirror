...
class FermionicGhostsLieConformalAlgebra(GradedLieConformalAlgebra):
    ...
    def __init__(self,R,ngens=2,names=None,index_set=None):
        r"""
        The Fermionic ghosts or `bc`-system Lie conformal algebra.

        INPUT:

        - ``R``: a commutative ring.
        - ``ngens``: an even positive Integer (default: ``2``); The 
          number of non-central generators of this Lie conformal algebra.
        ...
        if (names is None) and (index_set is None):
            ...
            self._latex_variable_names =  tuple(laxnames(ngens/2,'b') +\
