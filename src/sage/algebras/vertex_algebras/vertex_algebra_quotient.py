...
class VertexAlgebraQuotientElement(IndexedFreeModuleElement):
    ...
    def is_even_odd(self):
        """
        This returns ``1`` if this element is even and ``0`` if it is
        odd.
...
class VertexAlgebraQuotient(CombinatorialFreeModule):
    ...
    def lift(self, x):
        ...
        EXAMPLES::
            ...
            sage: V = vertex_algebras.Virasoro(QQ, 1/2); Q=V.quotient(V.ideal(V.find_singular(6)[0]))
            sage: L = Q(Q.gen(0))
            sage: v = L*(L*L)
            sage: v
