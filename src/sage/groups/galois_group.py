...
class _GaloisMixin(_GMixin):
    ...
    def stabilizer(self, x=None):
        r"""
        The pointwise stabilizer of an element of the splitting field (or of the top field).

        This is the subgroup corresponding to the subfield generated by that element
        under the Galois correspondence.

        INPUT:

        - ``x`` -- a subfield of the splitting field, specified by giving a generator.  Defaults to the top field.

        EXAMPLES::

            sage: R.<x> = ZZ[]
            sage: K.<a> = NumberField(x^3 - x + 1)
            sage: G = K.galois_group(names='b'); L = G.splitting_field()
            sage: G.stabilizer()
            Subgroup generated by [(1,4)(2,6)(3,5)] of (Galois group 3T2 (S3) with order 6 of x^3 - x + 1)

            sage: S = L.subfields(); M, fM, _ = S[1]; m = fM(M.gen())
            sage: m.minpoly().degree()
            2
            sage: G.stabilizer(m)
            Subgroup generated by [(1,2,3)(4,5,6)] of (Galois group 3T2 (S3) with order 6 of x^3 - x + 1)
        """
        if x is None:
            if self.is_galois():
                return self.subgroup([])
            L, emb = self._gcdata
            x = emb(self._field.gen())
            n = self.order() // self._field.degree()
        else:
            n = self.order() // x.minimal_polynomial().degree()
        H = self.subgroup([])
        if n == 1:
            return H
        gens = []
