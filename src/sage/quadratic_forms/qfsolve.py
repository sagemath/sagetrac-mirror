...
- Jeroen Demeyer (2014-09-23): use PARI instead of GP scripts (#16997),
  return vectors instead of tuples.
"""
...
def qfsolve(G):
    ...
    if ret.type() == 't_COL':
        return free_module_element(QQ, ret)
    return ZZ(ret)
...
def qfparam(G, sol):
    ...
    R = QQ['t']
    t = R.gen()
    mat = G._pari_().qfparam(list(sol))  # We need an explicit list(): #17028
    # Interpret the rows of mat as coefficients of polynomials
    return free_module_element(R, mat.Col())
