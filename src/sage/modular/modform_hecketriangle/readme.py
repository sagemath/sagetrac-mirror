...
- **Hecke triangle group:**
  ...
  triangle. The group is arithmetic if and only if `n=3, 4, 6, \infty`.
...
- **Decomposition into product of generators:**
  ...
  of the generators `S` and `T`. In particular this allows
...
- **Basic properties of group elements:**
  ...
  For the case `n=\infty` the last method is not
...
- **Fixed points:**
  ...
  one-to-one correspondence with primitive hyperbolic/parabolic group
  elements (at least if `n < \infty`). The group action on
...
- **Class number and class representatives**:
  ...
  Using the one-to-one correspondence with hyperbolic fixed points
...
- **Analytic type:**
  The analytic type of forms, including the behavior at infinity, are:
...
- **Modular form (for Hecke triangle groups):**
  ...
  The weight is a rational number of the form `4(nl+l')/(n-2) + (1-\epsilon)n/(n-2)`.
...
- **Hauptmodul:**
  ...
  from the hyperbolic triangle with vertices at `\rho = \mathrm{exp}(\pi/n)`, `i` and `\infinity` to the
  ...
  complex numbers. Another used normalization of `j`, denoted `J` in capital letters, is defined such that `J(i)=1`.
  The coefficients of `j` are rational numbers up to a power of `d=1/j(i)`
  which is only rational in the arithmetic cases `n=3, 4, 6, \infty`.
...
- **Basic generators:**
  There exist unique modular forms `f_\rho`, `f_i` and `f_{\infty}` such that
  each has a simple zero at `rho=exp(pi/n)`, `i` and `\infty` respectively and
  no other zeros. The forms are normalized such that their first Fourier coefficient
  is `1`. Their weight and multiplier are `(4/(n-2), 1)`, `(2*n/(n-2), -1)`,
  `(4*n/(n-2), 1)` respectively and can be defined in terms of the Hauptmodul `j`.
...
- **Eisenstein series and Delta:**
  The Eisenstein series of weight `2`, `4` and `6`, denoted `E_2, E_4` and `E_6`, exist for all `n` and
  are all implemented. Note that except for `n=3` the series `E_4` and `E_6`
  do not coincide with `f_{\rho}` and `f_i`.

  Similarly there always exists a (generalization of) `\Delta`, the modular discriminant. Note that except for ``n=3``
  it also does not coincide with ``f_{\infty}``.
  ...
  The case `n=\infty` is a special case (since there are two cusps)
...
- **Quasi modular form (for Hecke triangle groups):**
  `E_2` no longer transforms like a modular form but like a quasi modular form.
  More generally, quasi modular forms are given in terms of modular forms and powers
  of `E_2`. More precisely, a holomorphic quasi modular form is a sum of holomorphic modular
...
- **Ring of (quasi) modular forms:**
  ...
  `1/(n-2)\ZZ x \ZZ/(2\ZZ)` corresponding to the weight and multiplier.
  ...
  generators `f_{\rho}`, `f_i` and `E2`. The representations and arithmetic
...
- **Fourier expansion of (quasi) modular forms (for Hecke triangle groups):**
  ...
  i.e. the coefficient ring is given by `\mathrm{Frac}(R)(d)`.
...
- **L-functions of forms:**
  ...
  The conductor for the arithmetic groups `n = 3, 4, 6, \infty` is
...
- **Theta subgroup:**
  The Hecke triangle group corresponding to `n=\infty` is also
