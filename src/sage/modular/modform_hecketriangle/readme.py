...
- **Analytic type:**
  ...
  For ``n=infinity`` it is possible to specify whether fractional
  orders at the cusp ``-1`` (e.g. ``theta``) are allowed or not.
...
- **Generator for ``k=0``, ``ep=-1``:**
  ...
  In particular ``g_inv`` also exists for the Theta subgroup (``n=infinity``).
  ...
  EXAMPLES::
      ...
      sage: WeakModularFormsRing(n=infinity).g_inv()
      theta^4*f_i/(theta^8*d - f_i^2*d)
...
- **Theta subgroup:**
  ...
  In this case fractional orders at the cusp ``-1`` can occur
  (in particular for the ``theta`` function). Both the full space
  ...
  Also elements with fractional orders at ``-1`` are supported::

      sage: from sage.modular.modform_hecketriangle.space import ThetaWeakModularForms, ThetaQuasiModularForms, ModularForms
      sage: theta = ThetaWeakModularForms(k=0, ep=1).theta()
      sage: theta
      1 + 2*q + 2*q^4 + O(q^5)
      sage: E4 = ModularForms(n=infinity, k=4, ep=1).E4()
      sage: theta^8 == E4
      True
      sage: [v.reduce_d().as_ring_element() for v in ThetaQuasiModularForms(k=9/2).gens()]
      [(-23*theta^9 - 9*theta*f_i^2)/(-32),
       (theta^9 - theta*f_i^2)/64,
       theta*f_i*E2,
       theta*E2^2]
