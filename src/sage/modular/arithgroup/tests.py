...
class Test:
    ...
    def test_cong_closure(self):
        r"""
        Generate a random arithmetic subgroup, calculate its congruence
        closure, and check that the two are equal if and only if the Hsu
        congruence test returns True.
        ...
        Gc = G.congruence_closure()
        if not G.is_subgroup(Gc):
            raise AssertionError("Group %s not contained in its congruence closure" % G)
