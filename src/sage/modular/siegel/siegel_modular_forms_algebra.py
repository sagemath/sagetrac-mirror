...
class SiegelModularFormsAlgebraFactory(UniqueFactory):
    ...
    EXAMPLES::
        ...
        sage: S.construction()
        (SMFAlg{"Siegel Modular Group Sp(4,Z)", "even", 2, 101}, Integer Ring)
        sage: R.<a, b> = QQ[]
        sage: S = SiegelModularFormsAlgebra(coeff_ring=R)
        sage: S.construction()
        (SMFAlg{"Siegel Modular Group Sp(4,Z)", "even", 2, 101}, Multivariate Polynomial Ring in a, b over Rational Field)
    ...
    def create_object(self, version, key):
        ...
        TESTS::
            ...
            sage: key = SiegelModularFormsAlgebra.create_key(coeff_ring=QQ, group=Sp4Z_Gamma0(5), weights='all', degree=2, default_prec=41)
            sage: SiegelModularFormsAlgebra.create_object('1.0', key)
            Algebra of Siegel modular forms of degree 2 and all weights on Siegel Modular Group Gamma0(5) over Rational Field

        """
        C, R = key
        from sage.categories.pushout import CompositeConstructionFunctor, SiegelModularFormsAlgebraFunctor
...
class SiegelModularFormsAlgebra_class(Algebra):
    def __init__(self, coeff_ring=ZZ, group=Sp4Z, weights='even', degree=2, default_prec=SMF_DEFAULT_PREC):
        r"""
        ...
        if is_PolynomialRing(R):
            ...
            self.__base_ring = R
        Algebra.__init__(self, self.__base_ring)
    ...
    def gens(self, prec=None):
        ...
        TESTS::
            ...
            sage: S = SiegelModularFormsAlgebra(group=Sp4Z_Gamma0(2))
            sage: S.gens()
    ...
    def _coerce_map_from_(self, other):
        ...
        EXAMPLES::
            ...
            True
        """
        if self.__coeff_ring is other or self.base_ring().has_coerce_map_from(other):
            return True
        if isinstance(other, SiegelModularFormsAlgebra_class):
            if (self.group() == other.group()) or (other.group() == Sp4Z):
    ...
    def _element_constructor_(self, x):
        ...
        EXAMPLES::
            ...
            Traceback (most recent call last):
            ...
            TypeError: Unable to construct an element of Algebra of Siegel modular forms of degree 2 and even weights on Siegel Modular Group Sp(4,Z) over Rational Field corresponding to 2.67000000000000
...
def _siegel_modular_forms_generators(parent, prec=None, coefficient_degree=0):
    ...
    if group == Sp4Z and weights == 'even' and 2 == coefficient_degree:
