...
def _extract_embedded_signature(docstring, name):
    ...
    Using ``-Infinity`` as default argument is not parsed correctly,
    this is :trac:`18192`. But it doesn't raise an exception either::

        sage: _extract_embedded_signature("def foo(x=-Infinity)\n", "foo")
        ('def foo(x=-Infinity)\n', None)
    """
    ...
    # Parse the first line as "... name(args)"
    # This can fail for various reasons:
    # - the name is of the form ClassName.method_name
    # - the docstring was not generated by Cython
    m = re.search(name + " *[(].*[)]$", firstline)
    if m:
        signature = m.group()
    else:
        return docstring, None

    # Construct an artificial function with the same signature
    # (such that it can be parsed by Python)
    def_string = "def " + signature + ": pass"
    try:
        argspec = inspect.ArgSpec(*_sage_getargspec_cython(def_string))
    except Exception:
        return docstring, None
    docstring = L[1] if len(L) > 1 else '' # Remove first line, keep the rest
    return docstring, argspec
...
def _sage_getargspec_cython(source):
    ...
    if keywords is None:
        ...
        keywords = '**'+keywords
    def_string = "def dummy({}{}{}): pass".format(
            ''.join(py_units), varargs, keywords)
    return _sage_getargspec_from_ast(def_string)
