...
class FiniteDimensionalTwistedInvariantModule(SubmoduleWithBasis):
    ...
    def _repr_(self):
        ...
        if isinstance(self._ambient, Representation):
            M = M._module
        return f"Twist of ({self._group})-invariant submodule of {M}\
                 by character {self._chi.values()}"
        return f"Isotypic component of {self._group} acting on {M}\
                 corresponding to character {self._chi.values()}"
    ...
    def project(self, x):
        ...
        """
        return self.retract(self._project_ambient(x))
    ...
    # this should just return self._projection_matrix
