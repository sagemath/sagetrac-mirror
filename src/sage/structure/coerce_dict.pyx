   ...
   Note that ``MW`` also accepts values that do not allow for weak references::
    ...
    def __init__(self, size=11, data=None, threshold=0.7, weak_values=False):
        ...
        def eraser(r):
            ...
            cdef mono_cell* cursor = cself.lookup(<PyObject*><Py_ssize_t>r.key)
    ...
    def __contains__(self, k):
        ...
        if isinstance(r, KeyedRef) and PyWeakref_GetObject(r) == Py_None:
            ...
            return (not isinstance(value,KeyedRef)) or PyWeakref_GetObject(value) != Py_None
    ...
    cdef set(self,object k, object value):
        ...
        if self.weak_values:
            try:
                value_store = KeyedRef(value,self.eraser,<Py_ssize_t><void *>(k))
        ...
        if cursor.key_id == NULL or cursor.key_id == dummy:
            ...
            if cursor.key_id == NULL:
                ...
                maybe_resize = True
            try:
                key_store=KeyedRef(k,self.eraser,<Py_ssize_t><void *>(k))
    ...
    def pr(self):
        cdef size_t i = 0
        print "weak_values:",self.weak_values
        ...
        while i <= self.mask:
            ...
            if cursor.key_id == NULL:
                s0 = "-NUL-"
            elif cursor.key_id == dummy:
                s0 = "dummy"
            else:
                s0 = str(<long>cursor.key_id)
            s1 = "NULL" if (cursor.key_weakref == NULL) else (str(<object>cursor.key_weakref))
            s2 = "NULL" if (cursor.value == NULL) else (str(<object>cursor.value))
            print "%s %s %s %s"%(i,s0,s1,s2)
...
cdef class TripleDict:
    ...
    cdef triple_cell* lookup(self, PyObject* key1, PyObject* key2, PyObject* key3):
        ...
        cdef size_t key = <size_t>key1 + (<size_t>key2)<<2 + (<size_t>key3)<<4
    ...
    def __init__(self, size=11, data=None, threshold=0.7, weak_values=False):
        ...
        def eraser(r):
            cdef TripleDict cself = self
            if cself.table == NULL:
                return
                       
            k1,k2,k3 = r.key
            cdef triple_cell* cursor = cself.lookup(<PyObject*><Py_ssize_t>k1,<PyObject*><Py_ssize_t>k2,<PyObject*><Py_ssize_t>k3)
    ...
    cdef set(self, object k1, object k2, object k3, value):
        ...
        k = (<Py_ssize_t><void *>(k1),<Py_ssize_t><void *>(k2),<Py_ssize_t><void *>(k3))
        if self.weak_values:
            try:
                value_store = KeyedRef(value,self.eraser,k)
                entry.value = <PyObject*><void*>value_store
            except TypeError:
                entry.value = <PyObject*><void*>value
        else:
