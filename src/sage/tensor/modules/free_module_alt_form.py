...
class FreeModuleAltForm(FreeModuleTensor):
    ...
    def display(self, basis=None, format_spec=None):
        ...
        expansion = self._display_expansion(basis=basis,
                                            format_spec=format_spec)
        if self._name is None:
            resu_txt = repr(expansion)
        else:
            resu_txt = self._name + " = " + repr(expansion)
        if self._latex_name is None:
            resu_latex = latex(expansion)
        else:
            resu_latex = latex(self) + " = " + latex(expansion)
