...
def _strvector(obj):
    ...
    if len(not_none_names) == 0: # simple list
        ...
        return {
            'DATA': data,
            '_Names': [ r._r_to_sage_name(name) for (name, value) in obj.items() ],
...
def _list_vector(obj):
    ...
    return {
        ...
        'DATA': dict([(r._r_to_sage_name(key), r_to_sage_converter.ri2ro(val)) for (key, val) in obj.items()]),
        '_Names': [ r._r_to_sage_name(name) for (name, value) in obj.items() ],
        '_r_class': list(obj.rclass),
    };
...
class RElement(ExtraTabCompletion, ExpectElement):
    ...
    def __len__(self):
        ...
        EXAMPLES::
            ...
            5
        """
        return self.parent()('length(%s)'%self.name())._sage_()[0]
