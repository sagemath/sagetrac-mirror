...
def image_convert(files, format = "jpg"):
    """
    Converts to various image formats using PIL. Saves result in 
    SAGE_ROOT/tmp.

    NOTE:
      Converting to/from jpg requires, in addition, libjpeg. 
      This has not been tested on mac OS platforms.

    EXAMPLES:
        sage: from sage.media.pil import *
        sage: infile1 = "http://www.sagemath.org/old/art/.html/sage-car.png"
        sage: #infile1 = SAGE_ROOT+"/devel/sage/sage/media/pil-test-image1.png"
        sage: image_convert(infile1)
        sage: infile2 = "http://www.sagemath.org/old/art/.html/sage-icon7.png"
        sage: #infile2 = SAGE_ROOT+"/devel/sage/sage/media/pil-test-image2.png"
        sage: files = [infile1, infile2]
        sage: image_convert(files,"gif")
...
def image_blend(infile1, infile2, outfile, mask_ratio = 0.5):
    ...
    EXAMPLES:
        ...
        sage: infile2 = "http://www.sagemath.org/old/art/.html/sage-icon7.png"
        sage: #infile2 = SAGE_ROOT+"/devel/sage/sage/media/pil-test-image2.png"
        sage: outfile = SAGE_ROOT+"/tmp/pil-test-image1a.png"
        sage: image_blend(infile1, infile2, outfile)
    """
    if infile1[:4] == "http":
        im1 = urllib.URLopener()
        im2 = im1.retrieve(infile1)[0]
        mask = Image.open(im2)
    else:
        mask = Image.open(infile1)
    if infile2[:4] == "http":
        im1 = urllib.URLopener()
        im2 = im1.retrieve(infile2)[0]
        img = Image.open(im2)
    else:
        img  = Image.open(infile2)
...
def image_roll(infile, outfile, delta):
    ...
    EXAMPLES:
        ...
        sage: #infile = SAGE_ROOT+"/devel/sage/sage/media/pil-test-image1.png"
        sage: outfile = SAGE_ROOT+"/tmp/pil-test-image1a.png"
...
def image_id(files):
    ...
    EXAMPLES:
        ...
        sage: image_id(infile)[0][0][1:]
         ['PNG', '891x837', 'RGB']
        sage: image_id(infile)  # not tested - machine dependent path
        (/full/path/to/pil-test-image1.png, 'PNG', '1497x783', 'RGBA')
    ...
    for infile in files:
        try:
            if infile[:4] == "http":
                ...
                im = Image.open(infile)
            out.append([ [infile, im.format, "%dx%d" % im.size, im.mode] ])
        except IOError:
            pass
    return out
...
def image_filter(infile, outfile, filter_type="BLUR"):
    ...
    EXAMPLES:
        ...
        sage: infile2 = "http://www.sagemath.org/old/art/.html/sage-icon7.png"
        sage: #infile2 = SAGE_ROOT+"/tmp/pil-test-image1.jpg"
        sage: outfile = SAGE_ROOT+"/tmp/pil-test-image1a.jpg"
        sage: image_filter(infile2, outfile, filter_type="EDGE_ENHANCE_MORE")
        sage: outfile = SAGE_ROOT+"/tmp/pil-test-image1b.jpg"
        sage: image_filter(infile2, outfile, filter_type="BLUR")
        sage: outfile = SAGE_ROOT+"/tmp/pil-test-image1c.jpg"
...
def image_draw_text(infile, outfile, pt = (10, 10), text = None, font_type = "arial.pil", font_size = 18):
    """ 
...
def image_statistics(infile):
    ...
    EXAMPLES:
        ...
        [745767, 745767, 745767] [202.52117350325236, 213.39012184770846, 206.43753477962957] [254, 254, 254] [78.80113084931638, 62.111831253190921, 75.209232378752574]
    ...
    print stats.count, stats.mean, stats.median, stats.stddev
...
def image_autocontrast(infile, outfile):
    ...
    EXAMPLES:
        ...
        sage: #infile1 = SAGE_ROOT+"/devel/sage/sage/media/pil-test-image1.png"
        sage: image_convert(infile1, format = "jpg")
        sage: infile2 = "http://sage.math.washington.edu/home/wdj/art/thumbs/heart.jpg"
        sage: #infile2 = SAGE_ROOT+"/tmp/pil-test-image1.jpg"
        sage: outfile = SAGE_ROOT+"/tmp/pil-test-image1a.jpg"
    ...
    try:
        if infile[:4] == "http":
            ...
            im = Image.open(infile)
        ImageOps.autocontrast(im).save(outfile)
    except IOError:
