...
def _test_attrcall(name, L):
    """
    Return a function by name.

    This is a helper function for _test_finite_lattice(). This
    will unify all Boolean-valued functions to a function without
    parameters.

    EXAMPLES::

        sage: from sage.tests.finite_poset import _test_attrcall
        sage: N5 = posets.PentagonPoset()
        sage: N5.is_modular() == _test_attrcall('is_modular', N5)
        True
        sage: N5.is_constructible_by_doublings('convex') == _test_attrcall('is_doubling_convex', N5)
        True
    """
...
def _test_finite_lattice(L):
    ...
    EXAMPLES::
        ...
        sage: from sage.tests.finite_poset import _test_finite_lattice
        sage: L = posets.RandomLattice(10, 0.98)
        sage: _test_finite_lattice(L) is None  # Long time
        True
    """
    ...
    P = {x: _test_attrcall('is_' + x, L) for x in all_props}
    ...
    for p in sublattice_closed:
        if P[p] and not _test_attrcall('is_'+p, L_):
...
def _test_finite_poset(P):
    ...
    EXAMPLES::
        ...
        sage: from sage.tests.finite_poset import _test_finite_poset
        sage: P = posets.RandomPoset(10, 0.15)
        sage: _test_finite_poset(P) is None  # Long time
        True
    """
