...
def RandomGNP(n, p, bint directed=False, bint loops=False):
    ...
    - ``directed`` -- boolean (default:``False``); whether the random
      graph is directed or undirected (default)

    - ``loops`` -- boolean (default:``False``); whether the random
      digraph may have loops, or not (default). This value is used
    ...
    TESTS::
        ...
        ...
        ValueError: parameter 'loops' argument can be set to True only when 'directed' is True
    """
    from sage.graphs.graph import Graph, DiGraph

    # according the sage.misc.randstate.pyx documentation, random integers are
    # on 31 bits. We thus set the pivot value to p * 2 ^ 31
    ...
    if directed:
        ...
        G = Graph()
        if loops:
            raise ValueError("parameter 'loops' argument can be set to True only when 'directed' is True")
