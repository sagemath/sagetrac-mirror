...
def vertex_separation(G, algorithm="BAB", cut_off=None, upper_bound=None, verbose=False,
    ...
    if isinstance(G, Graph):
        ...
        if not G.is_strongly_connected():
            ...
            CC = [G.subgraph(scc) for scc in scc_digraph.topological_sort()[::-1]]
        else:
            CC = [G]
    ...
    # If the graph has several (strongly) connected components. We solve the
    # problem on each of them and order partial solutions in the same order than
    # in list CC. The vertex separation is the maximum over all these subgraphs.
    cdef int vs = 0
    cdef list L = []
    cdef int vsH
    cdef list LH
    cdef int this_cut_off = 0 if cut_off is None else cut_off
    for H in CC:

        if H.order() == 1:
            # We can directly add the vertex to the solution
            L.extend(H.vertex_iterator())

        else:
            # We get the vertex separation and corresponding ordering of this
            # (strongly) connected subgraph using desired algorithm
            if algorithm == "exponential":
                vsH, LH = vertex_separation_exp(H, verbose=verbose)

            elif algorithm == "MILP":
                vsH, LH = vertex_separation_MILP(H, verbosity=(1 if verbose else 0))

            elif algorithm == "BAB":
                vsH, LH = vertex_separation_BAB(H, cut_off=this_cut_off,
            ...
            this_cut_off = max(this_cut_off, vs)
