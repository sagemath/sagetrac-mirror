...
def from_edgelist_file(G,filename):
    r"""
    Build a (di)graph from an edgelist file.

    An edgelist file encodes a graph with an edge per line. Each edge may be two
    integer values separated by a space, or three values for a labelled
    (di)graph (the label is treated as a string).
    ...
    EXAMPLE::
        ...
        sage: from sage.graphs.graph_input import from_edgelist_file
        sage: s = "\n".join((str(u)+" "+str(v) for u,v in graphs.PetersenGraph().edges(labels=False)))
        sage: f = tmp_filename(ext='.edgelist')
        sage: open(f,'w').writelines(s)
        sage: g = Graph()
        sage: from_edgelist_file(g,f)
        sage: g == graphs.PetersenGraph()
        True
    """
    is_directed = G.is_directed()
    with open(filename,'r') as f:
        for line in f:
            ...
            if len(line) == 3:
                u,v,w = line
                G._backend.add_edge(int(u),int(v),w,is_directed)
            else:
                u,v = line
                G._backend.add_edge(int(u),int(v),None,is_directed)
...
def from_file(G,filename,format=None):
    ...
    - ``format`` (string) -- one of the following strings:
      ...
      - ``"edgelist"`` -- a file containing one pair 'u v' or a triple 'u v
        label' per line, representing an edge.
