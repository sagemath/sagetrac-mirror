...
cdef class gen(gen_auto):
    ...
    def __reduce__(self):
        ...
        """
        s = repr(self)
        return (lambda s: objtogen(self._pari, s), (s,))
    ...
    def gequal(gen a, b):
        ...
        WARNING: this relation is not transitive::
            ...
            False
        """
        cdef gen t0 = objtogen(a._pari, b)
        sig_on()
    ...
    def vecmax(x):
        ...
        """
        sig_on()
        return x._pari.new_gen(vecmax(x.g))
    ...
    def vecmin(x):
        ...
        """
        sig_on()
        return x._pari.new_gen(vecmin(x.g))
    ...
    def Col(gen x, long n = 0):
        ...
        """
        sig_on()
        return x._pari.new_gen(_Vec_append(gtocol(x.g), gen_0, n))
    ...
    def Ser(gen f, v=-1, long precision=-1):
        ...
        """
        if precision < 0:
            precision = f._pari.get_series_precision()
        sig_on()
        cdef long vn = f._pari.get_var(v)
        if typ(f.g) == t_VEC:
            # The precision flag is ignored for vectors, so we first
            # convert the vector to a polynomial.
            return f._pari.new_gen(gtoser(gtopolyrev(f.g, vn), vn, precision))
        else:
            return f._pari.new_gen(gtoser(f.g, vn, precision))
    ...
    def Strexpand(gen x):
        ...
        """
        if typ(x.g) != t_VEC:
            x = x._pari.vector(1, [x])
        sig_on()
        return x._pari.new_gen(Strexpand(x.g))
    ...
    def Strtex(gen x):
        ...
        if typ(x.g) != t_VEC:
            x = x._pari.vector(1, [x])
        sig_on()
        return x._pari.new_gen(Strtex(x.g))
    ...
    def Vec(gen x, long n = 0):
        ...
        """
        sig_on()
        return x._pari.new_gen(_Vec_append(gtovec(x.g), gen_0, n))
    ...
    def Vecrev(gen x, long n = 0):
        ...
        """
        sig_on()
        return x._pari.new_gen(_Vec_append(gtovecrev(x.g), gen_0, -n))
    ...
    def Vecsmall(gen x, long n = 0):
        ...
        """
        sig_on()
        return x._pari.new_gen(_Vec_append(gtovecsmall(x.g), <GEN>0, n))
    ...
    def padicprime(gen x):
        ...
        """
        sig_on()
        return x._pari.new_gen(gel(x.g, 2))
    ...
    def precision(gen x, long n=-1):
        ...
        sig_on()
        return x._pari.new_gen(precision0(x.g, n))
    ...
    def round(gen x, estimate=False):
        ...
        sig_on()
        if not estimate:
            return x._pari.new_gen(ground(x.g))
        y = x._pari.new_gen(grndtoi(x.g, &e))
        return y, e
    ...
    def truncate(gen x, estimate=False):
        ...
        sig_on()
        if not estimate:
            return x._pari.new_gen(gtrunc(x.g))
        y = x._pari.new_gen(gcvtoi(x.g, &e))
        return y, e
    ...
    def bernreal(x, unsigned long precision=0):
        ...
        """
        return x._pari.bernreal(x, precision)
    ...
    def besselk(gen nu, x, flag=None, unsigned long precision=0):
        ...
        cdef gen t0 = objtogen(nu._pari, x)
        sig_on()
        return nu._pari.new_gen(kbessel(nu.g, t0.g, prec_bits_to_words(precision)))
    ...
    def eint1(gen x, long n=0, unsigned long precision=0):
        ...
        sig_on()
        if n <= 0:
            return x._pari.new_gen(eint1(x.g, prec_bits_to_words(precision)))
        else:
            return x._pari.new_gen(veceint1(x.g, stoi(n), prec_bits_to_words(precision)))
    ...
    def polylog(gen x, long m, long flag=0, unsigned long precision=0):
        ...
        """
        sig_on()
        return x._pari.new_gen(polylog0(m, x.g, flag, prec_bits_to_words(precision)))
    ...
    def sqrtn(gen x, n, unsigned long precision=0):
        ...
        """
        cdef GEN zetan
        cdef gen t0 = objtogen(x._pari, n)
        sig_on()
        ans = x._pari.new_gen_noclear(gsqrtn(x.g, t0.g, &zetan, prec_bits_to_words(precision)))
        return ans, x._pari.new_gen(zetan)
    ...
    def sumdiv(gen n):
        ...
        """
        sig_on()
        return n._pari.new_gen(sumdiv(n.g))
    ...
    def sumdivk(gen n, long k):
        ...
        """
        sig_on()
        return n._pari.new_gen(sumdivk(n.g, k))
    ...
    def nfbasistoalg_lift(nf, x):
        ...
        EXAMPLES::
            ...
            -5/3*y^2 + 5/3*y - 1/6
        """
        cdef gen t0 = objtogen(nf._pari, x)
        sig_on()
        return nf._pari.new_gen(gel(basistoalg(nf.g, t0.g), 2))
    ...
    def bernvec(x):
        ...
        sig_on()
        return x._pari.new_gen(bernvec(x))
