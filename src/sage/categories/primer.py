...
feature using coercion (see :mod:`sage.structure.coerce` or the
thematic tutorial on "Coercion and Categories" for more
details).
...
``__mul__`` is a default implementation for all elements::

    sage: x.__mul__.__objclass__
    <type 'sage.structure.element.Element'>

It delegates the work to Sage's coercion model::

    sage: x.__mul__??                             # not tested

The coercion model asks the parent whether it acts on
itself by multiplication::

    sage: S.get_action(S, operator.mul, True)
    Left action by An example of a finite semigroup: the left regular band generated by ('a', 'b', 'c', 'd') on An example of a finite semigroup: the left regular band generated by ('a', 'b', 'c', 'd')
    sage: type(_)
    <type 'sage.structure.coerce_actions.MagmaStructure'>

If the two factors belong to different parents, then the coercion model
will first coerce them into a common parent. Then, it will call a single
underscore method ``_mul_()``. In our case, the method is defined as an
element method of the category of magmas::

    sage: x._mul_.__module__
    'sage.categories.magmas'

The inherited method does in fact delegate all the work to the parent::

    sage: x._mul_
    <bound method LeftRegularBand_with_category.element_class._mul_parent of 'a'>
    sage: x._mul_parent??     # not tested
    
The method ``x._mul_parent()`` calls the method ``product()`` of the parent,
which is `S` in our example::

    sage: S.product(x, x) == x*x
    True

Finally, the product was implemented for our example::

    sage: S.product.__module__
    'sage.categories.examples.finite_semigroups'

The general advice is: **Never override the double underscore multiplication
or addition methods! Instead, implement a single underscore method, or
inherit it from the category.**
