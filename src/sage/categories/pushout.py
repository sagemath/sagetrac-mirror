...
class SubspaceFunctor(ConstructionFunctor):
    ...
    EXAMPLES::
        ...
        sage: M_Z.<za,zb> = CombinatorialFreeModule(ZZ, ["a", "b"])
        sage: M_Q.<qa,qb> = CombinatorialFreeModule(QQ, ["a", "b"])
        sage: M_Q.coerce_map_from(M_Z)
        sage: M_Q.register_coercion(M_Z.hom([qa, qb]))
        sage: M_Q.coerce_map_from(M_Z)
        sage: S_Z = M_Z.submodule([za + 2 * zb]); S_Z
