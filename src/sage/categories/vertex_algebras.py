...
class VertexAlgebras(Category_over_base_ring):
    ...
    This is the base category of all vertex algebras. Subcategories with
    axioms are ``FinitelyGenerated`` and ``WithBasis``. 
    ...
    ``Super``. These functors commute::
        ...
        Category of H-graded super vertex algebras over Algebraic Real Field
        sage: VertexAlgebras(AA).Graded().Super() is VertexAlgebras(AA).Super().Graded()
        True

    That is, we only consider gradings on super vertex algebras that are
    compatible with the `\ZZ/2\ZZ` grading.
    ...
    def super_categories(self):
        ...
        """
        return [ProtoVertexAlgebras(self.base_ring()), LieConformalAlgebras(self.base_ring())]
    ...
    Graded = LazyImport("sage.categories.graded_vertex_algebras",
                        "GradedVertexAlgebras", "Graded")

    Super = LazyImport("sage.categories.super_vertex_algebras",
                       "SuperVertexAlgebras", "Super")

    WithBasis = LazyImport("sage.categories.vertex_algebras_with_basis",
                           "VertexAlgebrasWithBasis", "WithBasis")

    FinitelyGeneratedAsProtoVertexAlgebra = LazyImport(
        'sage.categories.finitely_generated_vertex_algebras',
        'FinitelyGeneratedVertexAlgebras')
