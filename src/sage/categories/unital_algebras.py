...
#        __imul__ = __mul__
    ...
    class WithBasis(CategoryWithAxiom_over_base_ring):
        ...
        class ParentMethods:
            ...
            def unital_subalgebra(self, elements, category=None, *args, **opts):
                """
                Return the unital subalgebra generated by ``elements`` and
                the unit of ``self``.
                ...
                EXAMPLES::
                    ...
                    sage: R.<x> = QQ[]
                    sage: Q = R.quo(x^2 - x)
                    sage: M = Q.submonoid(Q.gen())
                    sage: A = M.algebra(QQ, category=Monoids().Finite())
                    sage: b0,b1 = A.algebra_generators()
                    sage: A.one()
                    B[1]
                    sage: U = A.unital_subalgebra([b0], prefix='U')
                    sage: U.basis()
                    Finite family {0: U[0], 1: U[1]}
                    sage: S = A.subalgebra([b0], prefix='S')
                    sage: S.basis()
                    Finite family {0: S[0]}
                """
                elements = tuple(elements) + (self.one(),)
                category = UnitalAlgebras(self.category().base_ring()).or_subcategory(category)
                return self.subalgebra(elements, category, *args, **opts)
