...
from sage.combinat.partition import Partition, Partitions, _Partitions, is_weakly_decreasing
...
class SkewPartition(CombinatorialElement):
    ...
    def is_linked(self):
        ...
        Both row shape and column shape are valid::
            ...
            sage: SkewPartition([[2, 1], [1]]).is_linked()
            True

        Valid row shape but invalid column shape::

            sage: SkewPartition([[3, 2], [1]]).is_linked()
            False

        The empty skew-shape corresponds to empty (valid) partitions::

            sage: SkewPartition([[], []]).is_linked()
            True
    ...
    def right(self, row_index):
        ...
        EXAMPLES::
            ...
            sage: SkewPartition([[4, 1], [2]]).right(0)
            3

            sage: SkewPartition([[4, 1], [2]]).right(1)
            0

        An input index that is out of the bounds of the skew partition will throw an error::

            sage: SkewPartition([[4, 1], [2]]).right(2)
        ...
        An in-bounds index where no cells exist will return ``None``::
            ...
            sage: SkewPartition([[2, 1, 1, 1], [1, 1]]).right(1) == None
            True

        TESTS::

            sage: sp = SkewPartition([[1], []])
            sage: sp.right(0)
            0

            sage: sp = SkewPartition([[2], []])
            sage: sp.right(0)
            1

            sage: sp = SkewPartition([[1, 1], []])
            sage: sp.right(0)
            0

            sage: sp = SkewPartition([[2], [1]])
            sage: sp.right(0)
            1

            sage: sp = SkewPartition([[2], [2]])
            sage: sp.right(0) == None
            True
    ...
    def left(self, row_index):
        ...
        EXAMPLES::
            ...
            sage: SkewPartition([[4, 1], [2]]).left(0)
            2

            sage: SkewPartition([[4, 1], [2]]).left(1)
            0

        An input index that is out of the bounds of the skew partition will throw an error::

            sage: SkewPartition([[4, 1], [2]]).left(2)
            Traceback (most recent call last):
            ...
            IndexError: list index out of range
        ...
        An in-bounds index where no cells exist will return ``None``::
            ...
            sage: SkewPartition([[2, 1, 1, 1], [1, 1]]).left(1) == None
            True
    ...
    def top(self, col_index):
        ...
        EXAMPLES::
            ...
            sage: SkewPartition([[4, 1, 1], [2]]).top(0)
            2

        An in-bounds col_index where no cells exist will return ``None``::

            sage: SkewPartition([[4, 1, 1], [2]]).top(1) == None
            True

            sage: SkewPartition([[4, 1, 1], [2]]).top(2)
            0

            sage: SkewPartition([[4, 1, 1], [2]]).top(3)
            0

        A col_index that is out-of-bounds of the skew partition will throw an error::

            sage: SkewPartition([[4, 1, 1], [2]]).top(4)
            Traceback (most recent call last):
            ...
            IndexError: list index out of range
    ...
    def bottom(self, col_index):
        ...
        EXAMPLES::
            ...
            sage: SkewPartition([[4, 1, 1], [2]]).bottom(0)
            1

        An in-bounds col_index where no cells exist will return ``None``::

            sage: SkewPartition([[4, 1, 1], [2]]).bottom(1) == None
            True

            sage: SkewPartition([[4, 1, 1], [2]]).bottom(2)
            0

            sage: SkewPartition([[4, 1, 1], [2]]).bottom(3)
            0

        A col_index that is out-of-bounds of the skew partition will throw an error::

            sage: SkewPartition([[4, 1, 1], [2]]).bottom(4)
            Traceback (most recent call last):
            ...
            IndexError: list index out of range
    ...
    def k_boundary_to_partition(self, k=None, check=True):
        ...
        EXAMPLES::
            ...
            sage: SkewPartition([[3, 2, 1], [2, 1]]).k_boundary_to_partition()
            [3, 2, 1]

        If you input a skew-partition that is not a k-boundary to begin with, it will error::

            sage: SkewPartition([[3, 1], [2]]).k_boundary_to_partition(k=2)
            Traceback (most recent call last):
            ...
            AssertionError

        However, you may tell the method to *not* check that the input is a k-boundary::

            sage: SkewPartition([[3, 1], [2]]).k_boundary_to_partition(check=False)
            [3, 1]
    ...
    def is_k_boundary(self, k=None):
        ...
        EXAMPLES::
            ...
            sage: SkewPartition([[3, 2, 1], [2, 1]]).is_k_boundary()
            True
            sage: SkewPartition([[3, 2, 1], [2, 1]]).is_k_boundary(1)
            True
            sage: SkewPartition([[3, 2, 1], [2, 1]]).is_k_boundary(2)
            True
            sage: SkewPartition([[3, 2, 1], [2, 1]]).is_k_boundary(3)
            False
        ...
        if k is None:
            ...
            """
            l = sp.k_boundary_to_partition(check=False)
...
class SkewPartitions(UniqueRepresentation, Parent):
    ...
    def from_row_and_column_length(self, rowL, colL):
        """
        Construct a skew-partition from its row lengths and column lengths.
        ...
        - If it exists the unique skew-partition with row lengths ``rowL``
...
def row_shape_to_linked_skew_partitions(rs):
    ...
    Note that [4, 2, 1] / [1, 1] is *not* linked and hence doesn't appear in the list below::
        ...
        sage: from sage.combinat.skew_partition import row_shape_to_linked_skew_partitions
    ...
    TESTS::
        ...
        sage: p = []
        sage: row_shape_to_linked_skew_partitions(p)
        [[] / []]
        ...
        sage: p = [3, 2, 1]
        sage: row_shape_to_linked_skew_partitions(p)
        [[3, 2, 1] / [], [4, 2, 1] / [1], [5, 2, 1] / [2], [6, 3, 1] / [3, 1]]

        sage: p = [2, 2, 2]
        sage: row_shape_to_linked_skew_partitions(p)
        [[2, 2, 2] / [], [4, 2, 2] / [2], [6, 4, 2] / [4, 2]]
...
def size_to_linked_skew_partitions(size):
    ...
    EXAMPLES::
        ...
        sage: from sage.combinat.skew_partition import size_to_linked_skew_partitions
