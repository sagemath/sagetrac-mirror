...
def CombinatorialLogarithmSeries(R=QQ):
    ...
    has negative coefficients::
        ...
        doctest:...: DeprecationWarning: CombinatorialLogarithmSeries is deprecated, use CycleIndexSeriesRing(R).exponential() instead
    ...
    composition with `E^{+}` in both directions yields the multiplicative identity `X`)::
        ...
        [0, p[1], 0, 0]
    """
    deprecation(14846, "CombinatorialLogarithmSeries is deprecated, use CycleIndexSeriesRing(R).exponential() instead")
    return CycleIndexSeriesRing(R).omega()
