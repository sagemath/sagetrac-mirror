...
def Bracelets(n, k):
    ...
    EXAMPLES::
        ...
        print Bracelets(3, 2).cardinality()
        4
        print Bracelets(3, 2).first()
        [0, 0, 0]
        print Bracelets(3, 2).last()
        [1, 1, 1]
        print Bracelets(3, 2).list()
...
class Bracelets_NK(Parent):
    def __init__(self, n, k):
        ...
        super(Bracelets_NK, self).__init__(self, category = FiniteEnumeratedSets())
    ...
    def __contains__(self, x):
        ...
        if not all([isinstance(i, (int, Integer)) and i >= 0 and i < self.k for i in l]):
            return False
    ...
    def cardinality(self):
        ...
        """
        nk = sum([euler_phi(d) * self.k ** (self.n / d) for d in divisors(self.n)]) / self.n
    ...
    def __iter__(self):
        ...
        for i in self.GenB(1,1,1,-1,0,False):
            yield i

    
    def GenB(self, t, p, r, u, v, rs):
        """
        Algorithm for generating bracelets as described in 
            Sawada, Joe.  "Generating Bracelets in constant amortized time", SIAM
            Journal on Computing archive Volume 31 , Issue 1 (2001)
        """
        ...
        if t > self.n:
            ...
            if u != -1 and t == self.n and self.a[self.n] == self.a[1]:
                ...
                rev = self.CheckRev(t, u)
                if rev == 0:
                    for perm in self.GenB(t+1,p,r,u,v,rs):
                        yield perm
                if rev == 1:
                    for perm in self.GenB(t+1,p,t,u,v,False):
                        yield perm
            else:
                for perm in self.GenB(t+1,p,r,u,v,rs):
                    yield perm
            ...
            for j in range(self.a[t-p]+1, self.k):
                self.a[t] = j  
                for perm in self.GenB(t+1,t,r,u,0,rs):
                    yield perm
            
                
    def CheckRev(self, t, i):
