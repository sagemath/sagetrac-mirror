...
class DescendingPlanePartition():
    ...
    def _is_columnar_descending(self, dpp):
        """
        Return wether each column of the descending plane partition
    ...
    def sum(self):
        ...
        EXAMPLES::
            ...
            21
        """
        return sum(entry for row in self.DPP for entry in row)
    ...
    def number_of_regular_parts(self):
        ...
        A regular part of a descending plane partition is a part, `p`,
        of the descending plane partition such that `p` is greater
        than or equal to its position from the left of start of the
        row.
