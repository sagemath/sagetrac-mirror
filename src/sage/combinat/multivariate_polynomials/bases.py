...
def SchubertPolynomialsOnVectors(R, basis_name=None, basis_repr="Y", **keywords):
    ...
    .. MATH::
        ...
        Y_v = x_1^{v_1} x_2^{v_2} \cdots x_n^{v_n}

    if `v` is a partition,  i.e, if `v_1 \geq v_2 \geq \cdots \geq v_n`.
    
    Otherwise, we have for `v_i > v_{i+1}`

    .. MATH::

        Y_{\cdots v_{i+1} v_i-1 \cdots} = Y_v \partial_i,

    where $\partial_i$  is the `i`-th divided difference. 
...
def DemazurePolynomials(R, group_type="A", basis_name=None, basis_repr="K", **keywords):
    ...
    Here is the definition we use for type `A`.
    For `v = (v_1, \cdots, v_n) \in \NN^n`, we define
    
    .. MATH::

        K_v = x_1^{v_1} x_2^{v_2} \cdots x_n^{v_n}
    ...
    .. MATH::
        ...
        K_{\cdots v_{i+1} v_i \cdots} = K_v \pi_i,

    where `\pi_i` is the  `i`-th isobar divided difference. 
    ...
    - the ring of multivariate polynomials on x over ``R`` on the Demazure
      basis of type ``group_type``

    .. SEEALSO::

        :class:`AbstractPolynomialRing`
...
def DemazureHatPolynomials(R, group_type ="A", basis_name = None, basis_repr = "^K", **keywords):
    ...
    .. MATH::
        ...
        \hat{K}_v = x_1^{v_1}x_2^{v_2}\cdotsx_n^{v_n},
    ...
    `\hat{\pi}_i` is the `i`-th isobar hat divided difference. 
    ...
    - the ring of multivariate polynomials on x over ``R`` on the
      Demazure basis of type ``group_type``
...
def GrothendieckPolynomials(R, basis_name = None, basis_repr= "G", **keywords):
    ...
    Here is the definition we use. For`$v = (v_1, \cdots, v_n) \in \NN^n`,
    we define

    .. MATH::
    
        G_v = x_1^{v_1} x_2^{v_2} \cdots x_n^{v_n}
    ...
    .. MATH::
        ...
        G_{\cdots v_{i+1} v_i-1 \cdots} = G_v \pi_i,

    where `\pi_i` is the `i`-th isobar divided difference. 
    
    The vectors indexing the Grothendieck polynomials can as well
    been seen as lehmer codes.
