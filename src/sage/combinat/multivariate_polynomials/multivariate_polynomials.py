...
class AbstractPolynomialRing(UniqueRepresentation, Parent):
    ...
    - ``always_show_main_var`` -- if ``True``, then ``main_repr_var`` will be
      displayed on elements of every basis, even the ones that don't use it 
      directly (Schubert basis, Demazure basis, ...), false by default, used on
    ...
    def __init__(self, R, main_repr_var='x', bases_category_class=None,
        ...
        if bases_category_class is None:
            self._bases_category_class = Bases
        else:
            self._bases_category_class = bases_category_class
        
        if finite_bases_category_class is None:
            self._finite_bases_category_class = Finite_bases
        else:
            self._finite_bases_category_class = finite_bases_category_class
    
    def bases_category_class(self):
        r"""
        Return the class to use for the category of the ring bases
    ...
    def var(self, i, nb_variables=0):
        ...
        - the ``i``-th variable as a monomial element
        ...
        EXAMPLES::
            ...
            x[0, 0, 0, 1]
        """
        if nb_variables ==0 or nb_variables < i:
            nb_variables = i
    ...
    def _register_finite_ring(self,F):
        r"""
        Add the finite ring ``F`` as one of ``self`` finite rings. It is called
        ...
        """
        if F not in self._finite_rings:
            for ring in self._finite_rings:
                if ring.nb_variables() != F.nb_variables():
                    self._create_morphism(ring, F)
    ...
    def monomial_basis(self, basis_repr = None):
        ...
        if basis_repr is None:
            basis_repr = self._main_repr_var
    ...
    def ambient_space_basis(self, group_type, basis_repr = None):
        r"""
        Return the basis where polynomials are indexed by a root system lattice
    ...
    def from_morphism_basis(self, neutral_nb_variables, morphism_to_basis, get_basis_keys, get_morphism_on_basis, basis_name, basis_repr, variables_auto_coerce=False, **keywords):
        ...
        - ``neutral_nb_variables`` -- the default number of variables to
          get the one element
    ...
    def schubert_basis_on_vectors(self, basis_name = None, basis_repr = "Y"):
        r"""
        Creates the simple Schubert basis where Schubert polynomials are
        indexed by vectors.
    ...
    def demazure_basis_on_vectors(self, group_type ="A", basis_name = None, basis_repr = "K"):
        r"""
        Create the Demazure basis where demazure / key polynomials are indexed
        by vectors.
    ...
    def demazure_hat_basis_on_vectors(self, group_type ="A", basis_name = None, basis_repr = "^K"):
        r"""
        Create the Demazure hat basis where demazure polynomials are indexed
        by vectors.
        ...
        where `\pi_i` is the `i`-th isobar hat divided difference. 

        The vectors indexing the key polynomials can as well been
        seen as lehmer codes.
    ...
    def grothendieck_negative_basis_on_vectors(self, basis_name = None, basis_repr = "G"):
        r"""
        Create the simple Grothendieck basis where Grothendieck polynomials are indexed
        ...
        .. MATH::
            ...
            G_v = \prod_{1 \cdots n} (1 - \frac{1}{x_i})^{v_i}
    ...
    def grothendieck_positive_basis_on_vectors(self, basis_name=None, basis_repr="G"):
        r"""
        Create the simple Grothendieck basis where Grothendieck polynomials 
    ...
    def macdonald_basis_on_vectors(self, t1=None, t2=None, q=None, basis_name=None, basis_repr="M"):
        r"""
        Create the the basis of non-symmetric Macdonald polynomials indexed by vectors. 
    ...
    def _create_morphism(self,f1,f2):
        r"""
        Create a morphism between two `FiniteAbstractPolynomialRing` on their `FiniteMonomialBasis`
    ...
    def reduce_nb_variables(self, pol):
        """
        Create a polynomial by removing all last variables with exposant 0 of the given polynomial.
...
class FiniteAbstractPolynomialRing(UniqueRepresentation, Parent):
    ...
    def bases_category_class(self):
        r"""
        Return the class to use for he category of concrete bases.
    ...
    def monomial_basis(self, basis_repr=None):
        r"""
        Return the algebra ``self`` view in the monomials basis.
    ...
    def ambient_space_basis(self, letter, basis_repr=None):
        ...
        if letter == "A":
            number = self.nb_variables() - 1
        else:
            number = self.nb_variables()
