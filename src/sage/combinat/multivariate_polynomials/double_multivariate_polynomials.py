...
class DoubleAbstractPolynomialRing(AbstractPolynomialRing):
    r"""
    This class implements the ring of abstract polynomials over a double
    set of variables.

    This ring is actually an :class:`AbstractPolynomialRing` over another
    :class:`AbstractPolynomialRing` but the class provides methods that
    are specific to double multivariate polynomials to make it easy to use.

    INPUT:

    - ``R`` -- the base ring
    - ``repr_var1`` -- string representing the main variable set, default is `x`
    - ``repr_var2`` -- string representing the secondary variable set, default is `y`
    - ``inversed_ring``, the ring where the roles of the two sets of variables are inversed

    By default, nothing is sent and the ring is created, the field
    is then used to avoid infinite recursion.
            
    OUTPUT:

    - the abstract ring of multivariate polynomials on ``repr_var1`` over the
      abstract ring of multivariate polynomials on ``repr_var2`` over ``R``
    ...
    All the bases and actions existing in :class:`AbstractPolynomialRing`
    are available, they correpond to the bases and actions on the `x`
    set of variables::
    ...
    Be carreful as the upper case `Y` here doesn't correspond to the
    set of variables but reprensents the Schubert polynomials.
    ...
    def coeffs_ring(self):
        r"""
        Return the multivariate polynomial ring on the second set of variables
    ...
    def inversed_ring(self):
        r"""
        Return the ring of multivariate polynomials where the roles of
        the two sets of variables are exchanged.
...
class DoubleSchubertBasisOnVectors(LinearBasisOnVectors):
    ...
    def on_basis_method(self, x, basis, call_back):
        ...
        return prod([basex.var(i+1) - basey.var(j+1) for i in xrange(len(x))
                     for j in xrange(x[i])], basex.one())
