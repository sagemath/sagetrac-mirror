    ...
    for i in range(k): # For any column C1
        ...
        for j in range(i+1,k): # For any column C2 > C1
            ...
            if bitset_len(seen) != n2: # Have we seen all pairs ?
                ...
                if verbose:
                    if terminology=="OA":
                        print "Columns {} and {} are not orthogonal".format(i,j)
                    else:
                        if i==0:
                            if j==1:
                                print "The row and column indices are not orthogonal"
                            else:
                                print "Square {} is not row latin".format(j-2)
                        elif i==1:
                            print "Square {} is not column latin".format(j-2)
                        else:
                            print "Squares {} and {} are not orthogonal".format(i-2,j-2)
                return False
