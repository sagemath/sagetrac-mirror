...
def one_radical_difference_family(K, k):
    ...
    even, let us introduce the following notations that are the one we use in
    the algorithm:
    ...
    root of unity for `k` even. And `H^{2mt} = \langle r \rangle`.

    It is easy to check (see [Wi72]_) that for `k` odd `\Delta H^{2mt}
    ...
    translates of the unique tile `A`. This tiling problem is solved with
    :func:`one_cyclic_tiling`.
    """
    q = K.cardinality()
    x = K.multiplicative_generator()
