...
def find_recursive_construction(k,n,existence=False):
    ...
    EXAMPLES::
        ...
        doctest:1584: RuntimeWarning: Currently, setting min_part=0 produces Composition objects which violate internal assumptions.  Calling methods on these objects may produce errors or WRONG results!
        sage: print count
        40
        sage: len([x for x,v in find_construction_3_3.cache.items() if v]),len(find_construction_3_3.cache)
        (21, 140)
        sage: len([x for x,v in find_construction_3_4.cache.items() if v]),len(find_construction_3_4.cache)
        (17, 119)
        sage: len([x for x,v in find_construction_3_5.cache.items() if v]),len(find_construction_3_5.cache)
        (2, 102)
        sage: len([x for x,v in find_construction_3_6.cache.items() if v]),len(find_construction_3_6.cache)
        (0, 100)
    """
...
def find_construction_3_3(k,n):
    ...
    """
    for nn in range(2,n//2+1):
        for mm in range(2,n//nn+1):
            i = n-nn*mm
            if i<=0:
                continue

            if (orthogonal_array(k+i, nn , existence=True) and # master design 3.3
                orthogonal_array( k , mm , existence=True) and
                orthogonal_array( k ,mm+1, existence=True) and
    ...
    for nn in range(2,n//2+1):
        for mm in range(2,n//nn+1):
            ...
            for s in range(1,min(i,nn)):
                r = i-s
                if (orthogonal_array(k+r+1,nn,existence=True) and # 3.4
                    orthogonal_array(k    , s,existence=True) and
                    orthogonal_array(k,mm+0,existence=True) and
                    orthogonal_array(k,mm+1,existence=True) and
                    orthogonal_array(k,mm+2,existence=True) and
...
def find_construction_3_5(k,n):
    ...
    for nn in range(2,n//2+1):
        for mm in range(2,n//nn+1):
            ...
            if (orthogonal_array(k+3,nn,existence=True) and
                orthogonal_array(k,mm+3,existence=True) and
                orthogonal_array(k,mm+1,existence=True) and
                orthogonal_array(k,mm+2,existence=True)
                #and holes(k,mm+3,existence=True)
                ):

                for r,s,t in Compositions(i,length=3,max_part=nn-1,min_part=0):
                    if (r <= s and
                        (nn-r-1)*(nn-s) < t and
...
def find_construction_3_6(k,n):
    ...
    for nn in range(2,n//2+1):
        for mm in range(2,n//nn+1):
            ...
            if (is_prime_power(nn) and                      # 3.6
                orthogonal_array(k+i,nn,existence=True) and
                orthogonal_array(k,mm+0,existence=True) and
                orthogonal_array(k,mm+1,existence=True) and
                orthogonal_array(k,mm+2,existence=True)):
...
def OA_and_oval(q):
    ...
    #
    #B.sort(key=lambda b:int(x in b)*sum([xx in oval for xx in b])) ------------- when it passes tests remove this line !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
