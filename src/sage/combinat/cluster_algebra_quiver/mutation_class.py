...
def _digraph_mutate(dg, k, n, m):
    """
    Return a digraph obtained from ``dg`` with ``n+m`` vertices by
    mutating at vertex ``k``.
    ...
    TESTS::
        ...
        sage: B = matrix(3,[0,1,-1])
        sage: Q = ClusterQuiver(B)
        sage: dg = Q.digraph()
        sage: _digraph_mutate(dg,0,1,2).edges()
        [(0, 1, (1, -1)), (2, 0, (1, -1))]
    """
    edges = {(v1, v2): label
             for v1, v2, label in dg._backend.iterator_in_edges(dg, True)}
    ...
    for (v1, _, label1) in in_edges:
        if v1 > n:
            continue
        l11, l12 = label1
        for (_, w2, label2) in out_edges:
            if w2 > n:
                continue
            l21, l22 = label2
            if (v1, w2) in edges:
    ...
    new_edges += [(v1, v2, edges[(v1, v2)])
                  for (v1, v2) in edges
                  if not (v1, v2, edges[(v1, v2)]) in del_edges]

    dg_new = DiGraph()
    dg_new.add_vertices(range(n + m))
