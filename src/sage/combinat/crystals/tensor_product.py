...
class TensorProductOfCrystalsElement(ImmutableListWithParent):
    ...
    def _repr_diagram(self):
        ...
        for c in self:
            ...
            max_widths.append(max([len(x) for x in pplist[-1]]))
        num_rows = max([len(x) for x in pplist])
        ret = ""
...
#        super(CrystalOfTableaux, self).__init__(category = FiniteEnumeratedSets())
    ...
    def module_generator(self, shape):
        ...
        if invert:
            ...
            module_generator = [f(x) for x in module_generator]
...
#            list=Tableau(rows).to_word_by_column()
        ...
        TensorProductOfRegularCrystalsElement.__init__(self, parent, [parent.letters(x) for x in list_])
