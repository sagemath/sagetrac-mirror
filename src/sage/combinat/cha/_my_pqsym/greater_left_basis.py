# -*- coding: utf-8 -*-
r"""
The left elementary basis of PQSym for Weak Order
"""

from my_pqsym import ParkingQuasiSymmetricFunctions

class GreaterLeft(ParkingQuasiSymmetricFunctions.Bases.Base):
    '''
...
#        return self.realisation_of().GLD()
    ...
    def morph_L_GL(self):
        ...
        GL_to_L = morph(GL, L,
            lambda sigma: L.sum_of_monomials(
                ParkingFunction_class(list(sigma)).multi_left_permutohedron_greater()),
            tri="lower")
    ...
    def morph_SR_GL(self):
        ...
        GL._module_morphism(
            lambda sigma: SR(ParkingFunction(sigma[::-1])),
            codomain=SR
        ).register_as_coercion()

        SR._module_morphism(
            lambda sigma: GL(ParkingFunction(sigma[::-1])),
            codomain=GL
        ).register_as_coercion()
