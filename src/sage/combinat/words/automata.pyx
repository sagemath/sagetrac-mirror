...
#            print( "Error : incompatible keys : %s and %s"%(d,dA))
    ...
    # retourne un automate dont le langage est w^(-1)L (self est supposé déterministe)
    def prefix(self, w, i=None, i2=None, verb=False): 
        if i is None:
            if hasattr(self, 'I'):
                i = self.I[0]
            else:
...
#                        raise ValueError("the automaton must be complete")
        ...
        if puit is not None:
            if verb:
                print("effacement de l'etat puit %s" % puit)
