...
class Fundamental(WordQuasiSymmetricFunctions._Basis):
    """
    ...
    def left_coproduct_on_basis(self, pw):
        ...
        l = list(pw); l.reverse()
        id_max = pw.grade() - l.index(max(pw)) - 1
        return self.tensor_square().sum_of_monomials(
            [(to_pack(pw[:i+1]), to_pack(pw[i+1:]))
            for i in range(id_max, pw.grade()-1)]
        )
    ...
    def right_coproduct_on_basis(self, pw):
        ...
        return self.tensor_square().sum_of_monomials(
            [(to_pack(pw[:i+1]), to_pack(pw[i+1:]))
            for i in range(min(id_max, pw.grade()))]
        )