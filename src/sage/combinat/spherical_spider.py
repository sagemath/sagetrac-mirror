...
class FreeSphericalSpider(CombinatorialFreeModule):
    ...
    def __classcall__(cls, base_ring, boundary):
        ...
        return super(FreeSphericalSpider, cls).__classcall__(cls, base_ring, tuple(boundary))
    ...
    def rotate(self, k):
        ...
        """
        b = self.boundary()
        codomain = FreeSphericalSpider(self.base_ring(), b[k:]+b[:k])
...
class LinearSphericalSpider(FreeSphericalSpider):
    ...
    """
    @staticmethod
    def __classcall__(cls, base_ring, boundary, rewrite_rules=None):
        if boundary in NN:
            boundary = [Strand()]*boundary

        return super(FreeSphericalSpider, cls).__classcall__(cls, base_ring, tuple(boundary), rewrite_rules)

    def __init__(self, base_ring, boundary, rewrite_rules):
        r"""
        Initialise ``self``.`
        ...
        EXAMPLES::
            ...
            True
        """
        FreeSphericalSpider.__init__(self, base_ring, boundary)
...
class WebAlgebra(CombinatorialFreeModule):
    ...
    def __init__(self, base_ring, boundary):
        ...
        """
        CombinatorialFreeModule.__init__(self, base_ring, SphericalSpider(boundary),
            category=Algebras(base_ring).WithBasis().Unital())
    ...
    def markov_trace(self):
        ...
        codomain = FreeSphericalSpider(self.base_ring(),[])
...
def from_diagram(n, delta):
    ...
    EXAMPLES::
        ...
        2*B[The spherical web with c = (1, 0, 3, 2) and e = ().] + 2*B[The spherical web with c = (3, 2, 1, 0) and e = ().]
    """
