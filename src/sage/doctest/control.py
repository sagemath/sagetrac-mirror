...
class DocTestDefaults(SageObject):
    ...
    ``$SAGE_SCRIPTS_DIR/sage-runtests``, expect for a few places, which is
    mostly to make doctesting more predictable.
...
class DocTestController(SageObject):
    ...
    def __init__(self, options, args):
        ...
        - ``options`` -- either options generated from the command line by
          ``$SAGE_SCRIPTS_DIR/sage-runtests`` or a DocTestDefaults object
          (possibly with some entries modified)
    ...
    def run_val_gdb(self, testing=False):
        ...
        expand them here and risk insufficient quoting. ::
            ...
            exec gdb -x "$SAGE_SCRIPTS_DIR/sage-gdb-commands" --args python "$SAGE_SCRIPTS_DIR/sage-runtests" --serial --timeout=0 hello_world.py
        ...
        opt = self.options
        if opt.gdb:
            cmd = '''exec gdb -x "$SAGE_SCRIPTS_DIR/sage-gdb-commands" --args '''
            flags = ""
            if opt.logfile:
