...
def echelonize_cdv_exact(M, transformation, integral, secure):
    ...
    - ``transformation`` -- a boolean; whether the transformation matrix is
      returned
    ...
    If `r` is the rank of `M`, the first `r` columns of the echelon form are
    exact, i.e. given at the maximal precision allowed by the parent.
