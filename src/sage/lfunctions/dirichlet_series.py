...
class DirichletSeries(SageObject):
    ...
    def _check_construction(self):
        """
        Raise an error if we cannot construct the Dirichlet series from self.

        TESTS::

            sage: s = var('s')
            sage: dirichlet_series(s/2)     # indirect doctest
            Traceback (most recent call last):
            ...
            NotImplementedError: Cannot construct Dirichlet series from the factor 1/2
            sage: dirichlet_series(sin(s))     # indirect doctest
            Traceback (most recent call last):
            ...
            NotImplementedError: Cannot construct Dirichlet series from the factor sin(s)
            sage: dirichlet_series(s^s)     # indirect doctest
            Traceback (most recent call last):
            ...
            NotImplementedError: Cannot construct Dirichlet series from the factor s
        """
        ok = True
        try:
    ...
    class MaskFunction(BuiltinFunction):
        def __init__(self):
            """
            Internally used function object.
    ...
    def _is_zeta_expr(ex, var):
        ...
        if (ex.operator() == zeta and len(ex.operands()) == 1
            ...
            d = arg.match(w0 * var)
            if d is not None:
                factor = list(d.values())[0]
                return factor.is_integer() and bool(factor > 1)
            d = arg.match(var + w0)
            if d is not None:
                summand = list(d.values())[0]
    ...
    def _is_exp_expr(ex, v):
        """
        Return True if ``ex`` is a viable exponential term in ``v``.

        TESTS::

            sage: s = var('s')
            sage: dummy = dirichlet_series.MaskFunction()
            sage: assert(dirichlet_series._is_exp_expr(dummy(2, -s), s))
            sage: assert(dirichlet_series._is_exp_expr(dummy(2, -s), s))
            sage: assert(not dirichlet_series._is_exp_expr(dummy(2, s), s))
