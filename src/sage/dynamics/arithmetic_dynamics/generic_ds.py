...
class DynamicalSystem(SchemeMorphism_polynomial,
    ...
    def is_prime_of_good_reduction(self,prime):
        ...
        try:
            ...
            # the mod fucntion throws an exception only if
            # the polynomials of the DynamicalSystem dont have the same degree which
            # means that the prime is a prime of bad reduction
            return False
        # check if the degree of the DynamicalSystem has changed.
        # if yes then the prime is a prime of bad reduction
        # if no then the prime is a prime of good reduction
