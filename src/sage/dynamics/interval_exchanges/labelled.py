...
class LabelledPermutation(SageObject):
    ...
    def __init__(self, intervals=None, alphabet=None):
        ...
        if intervals is None:
            ...
            self._intervals = [
                [self._alphabet.rank(x) for x in intervals[0]],
                [self._alphabet.rank(x) for x in intervals[1]]]
    ...
    def __getitem__(self,i):
        ...
        TESTS::
            ...
            'a'
        """
        return [self._alphabet.unrank(x) for x in self._intervals[i]]
    ...
    def list(self):
        ...
        Recovering the permutation::
            ...
            True
        """
        a0 = [self._alphabet.unrank(x) for x in self._intervals[0]]
        a1 = [self._alphabet.unrank(x) for x in self._intervals[1]]
        return [a0, a1]
...
class FlippedLabelledPermutation(LabelledPermutation):
    ...
    def list(self, flips=False):
        ...
        """
        if flips:
            a0 = zip([self._alphabet.unrank(x) for x in self._intervals[0]], self._flips[0])
            a1 = zip([self._alphabet.unrank(x) for x in self._intervals[1]], self._flips[1])
        else:
            a0 = [self._alphabet.unrank(x) for x in self._intervals[0]]
            a1 = [self._alphabet.unrank(x) for x in self._intervals[1]]
    ...
    def __getitem__(self,i):
        ...
        letters = [self._alphabet.unrank(x) for x in self._intervals[i]]
        flips = self._flips[i]
