...
def julia_plot(c=-1,
    ...
    Define the map `Q_c(z) = z^2 + c` for some `c \in \mathbb{C}`. Let `R_e = 
    \bigl(1 + \sqrt{1 + 4|c|}\bigr)^2/4`. For every `p \in \mathbb{C}`, if 
    `|Q_{c}^{k}(p)| > R_e` for some `k \geq 0`, then `Q_{c}^{n}(p) \to \infty`. 
    Let `N` be the maximum number of iterations. Compute the first `N` points on 
    the orbit of `p` under `Q_c`. If for any `k < N`, `|Q_{c}^{k}(p)| > R_e`, we 
    stop the iteration and assign a color to the point `p` based on how quickly 
    `p` escaped to infinity under iteration of `Q_c`. If `|Q_{c}^{i}(p)| \leq R_e` 
