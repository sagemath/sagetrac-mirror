...
cdef class MarchingCubesTriangles(MarchingCubes):
    ...
    cdef _update_yz_vertices(self, int x, np.ndarray _prev, np.ndarray _cur, np.ndarray _next):
        """
        TESTS::
    ...
    cdef _update_x_vertices(self, int x, np.ndarray _prev, np.ndarray _left, np.ndarray _right, np.ndarray _next):
        """
        TESTS::
    ...
    cdef process_cubes(self, np.ndarray _left, np.ndarray _right):
        """
        TESTS::
...
#      *              Z
    ...
    cdef add_triangle(self, VertexInfo v1, VertexInfo v2, VertexInfo v3):
        """
...
cdef render_implicit(f, xrange, yrange, zrange, plot_points, cube_marchers):
    """
    INPUT:
