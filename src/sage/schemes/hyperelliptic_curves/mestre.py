...
def HyperellipticCurve_from_invariants(i, reduced=True, precision=None,
                                       algorithm='default'):
    r"""
    Returns a hyperelliptic curve with given Igusa-Clebsch invariants

    INPUT:

    - ``i`` - list or tuple of length 4 containing the four Igusa-Clebsch
      invariants: I2,I4,I6,I10
    - ``reduced`` - Bolean (default = True) If True, tries to reduce the
    ...
    Examples over the rationals::
        ...
        ...
        NotImplementedError: Reduction of hyperelliptic curves not yet implemented. See trac #5534 and #14756.
    ...
    This algorithm is based on the formulae of [LT2001]_ page 957
    cross-referenced with [Wam]_ to correct typos.

    Construct Mestre's conic using the :func:`Mestre_conic` function. Try to
    parameterize all rational points on the conic, note that this is only
    possible if the conic has no rational points in which case return a message
    saying so. Let `f1, f2, f3` be the three coordinates of the parametrization
    of the conic by the projective line, and change them into one variable by
    letting `F_i = f_i(t, 1)`. Note that each `F_i` has degree at most 2.
    Construct a sextic polynomial `f = \sum_{0<=i,j,k<=3}{c_ijk*F_i*F_j*F_k}`,
    where `$c_ijk$` are defined as rational functions in the invariants
    (see the source code for detailed formulae for `c_ijk`).
    If reduced is True, then reduce `f` using the :func:`reduce_polynomial`
    function. Finally construct a hyperelliptic curve from `f` by using the
    constructor :func:`~sage.schemes.hyperelliptic_curves.HyperellipticCurve`

    REFERENCES:

    - [LY2001] K. Lauter, T. Yang "Journal of Number Theory 131 (2011)" pages
      936 - 958
    - [Wam] P. van Wamelen. Pari Code, section "thecubic"
...
def Mestre_conic(i, xyz=False, names='u,v,w'):
    ...
    The following algorithm is taken from pages 956 - 957 of [LY2001]_ and is
    based on [ME1991]_ pages 321,332
    ...
    - [LY2001] K. Lauter, T. Yang "Journal of Number Theory 131 (2011)" pages
      936 - 958
    - [ME1991] J-F Mestre "Construction de courbes de genre 2 a partir de leurs
      modules"  pages 314 - 334
    """
    from sage.structure.sequence import Sequence
