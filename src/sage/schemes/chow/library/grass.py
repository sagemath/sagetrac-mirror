...
def GrassBundle(A, B, chern_class='c', names=None, name=None, latex_name=None):
    ...
    if isinstance(A, Sheaf):
        E, n = A, A.rank()
        r = int(B) if is_Integer(B) else B
        ...
        E, n = B, B.rank()
        k = int(A) if is_Integer(A) else A
    ...
    QZ = Bundle(Z, r, [1] + new_vars)  # if s==k : c_{s+1} = ... = c_{r} = 0
...
def Grass(n, r, chern_class='c', names=None, name=None, latex_name=None):
    ...
    """
    n = int(n) if is_Integer(n) else n
    r = int(r) if is_Integer(r) else r
