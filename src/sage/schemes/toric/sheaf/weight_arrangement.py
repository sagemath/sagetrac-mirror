...
def make_WeightArrangement(hyperplane_arrangement):
    ...
    if parent.base_ring().characteristic():
...
class WeightArrangement(object):
    ...
    def bounded_regions_points(self):
        ...
        hyperplane arrangements::
            ...
            sage: b_regions = H.bounded_regions()
            sage: sum(len(region.integral_points()) for region in b_regions)
            11
        ...
        return tuple(sorted([tuple(sorted(points))
                             for points in points_by_sign.values()]))
