...
.. NOTE::
    ...
    This code could probably get moved into Sage at some point,
...
def ap(E, p):
    ...
    .. WARNING:: This should be really slow.
    """
    k = p.residue_field()
    t = E.local_data(p)
...
def primes_of_bounded_norm(F, B):
    r"""
    Return an iterator over all prime ideals of norm `\leq B` in the ring
    ...
    for nm in sorted(v.keys()):
        X = v[nm]
        if len(X) and ZZ(nm).is_prime_power():
            if X[0].is_prime():
                for p in X:
                    yield p
...
def ap_dict(E, B):
    ...
    K = E.base_field()
    P = K.primes_of_bounded_norm(B)
    return {reduced_rep(p): ap(E, p) for p in P}
            ...
            if reduced_rep(I) in an:
                ...
                if len(fac) == 0:
                    ...
                    if p.divides(N):
                        ...
                        assert e >= 2
                        an[reduced_rep(I)] = (an[reduced_rep(p)] *
                                              an[reduced_rep(p**(e - 1))]
                                              - p.norm() *
                                              an[reduced_rep(p**(e - 2))])
    return an


def an_dict(E, B):
    r"""
    Given an elliptic curve `E` over a number field, return dictionary
    ...
    dictionary mapping reduced rep of ideals (N(p),p.reduced_gens()) to integers
...
def _test_an_dict_over_Q(ainvs, B=100):
    ...
    EXAMPLES::
        ...
        sage: from sage.schemes.elliptic_curves.aplist_nf import _test_an_dict_over_Q
        sage: _test_an_dict_over_Q([0,1,1,0,0], B=100)
    """
    from sage.all import polygen, QQ, NumberField, EllipticCurve
    x = polygen(QQ, 'x')
    F = NumberField(x - 1, 'a')
...
def test_an_dict_over_Q():
    ...
    EXAMPLES::
        ...
        sage: from sage.schemes.elliptic_curves.aplist_nf import test_an_dict_over_Q
        sage: test_an_dict_over_Q()
    """
