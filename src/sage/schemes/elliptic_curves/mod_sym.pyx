...
ctypedef public long long llong
...
cdef llong llsign(llong n) except -2:
    r"""
    Returns the sign of a long long
    """
    if n < 0:
        return -1
    return 1
...
cdef llong llxgcd(llong a, llong b, llong *ss, llong *tt) except -1:
    r"""
    Computes the greatest common divisor `g` of `a` and `b`,
...
def _test_llfunctions(a,b):
    ...
    EXAMPLES::
        ...
        sage: from sage.schemes.elliptic_curves.mod_sym import _test_llfunctions
...
def _test_proj_normalise(N,u,v):
    ...
    EXAMPLES::
        ...
        sage: from sage.schemes.elliptic_curves.mod_sym import _test_proj_normalise
...
def _test_best_proj_point(u,v,N):
    ...
    EXAMPLES::
        ...
        sage: from sage.schemes.elliptic_curves.mod_sym import _test_best_proj_point
...
cdef class CuspsForModularSymbolNumerical:
    r"""
...
def _test_cusps(r,N):
    ...
    EXAMPLES::
        ...
        sage: from sage.schemes.elliptic_curves.mod_sym import _test_cusps
...
cdef class ModularSymbolNumerical:
    ...
    EXAMPLES::
        ...
        sage: M(77/57)
        -1
        sage: M = E.modular_symbol(sign=-1, method="num")
        sage: M(2/7)
        2
    ...
    def __init__(self, E, sign=+1):
        ...
        EXAMPLES::
            ...
            sage: from sage.schemes.elliptic_curves.mod_sym import ModularSymbolNumerical
            sage: M = ModularSymbolNumerical(EllipticCurve("27a1"))
            sage: M(1/9)
            1/2
            sage: M(1/3)
            -1/6
...
# == basics ================
    ...
    def __repr__(self):
        ...
        EXAMPLE::
            ...
            sage: from sage.schemes.elliptic_curves.mod_sym import ModularSymbolNumerical
            sage: M = ModularSymbolNumerical(EllipticCurve("14a1"))
            sage: M
    ...
    def elliptic_curve(self):
        ...
        EXAMPLE::
            ...
            sage: from sage.schemes.elliptic_curves.mod_sym import ModularSymbolNumerical
            sage: M = ModularSymbolNumerical(EllipticCurve("15a4"))
    ...
    def __call__(self, r, int sign=0, use_twist=True):
        ...
        EXAMPLES::
            ...
            sage: from sage.schemes.elliptic_curves.mod_sym import ModularSymbolNumerical
            sage: M = ModularSymbolNumerical(EllipticCurve("36a1"))
            sage: M(2/5)
            -1/3
            sage: M(2/5, -1)
            1/2

            sage: M = ModularSymbolNumerical(EllipticCurve("54a1"))
            sage: M(5/9)
            -1/2
...
# == initialisation ========
    ...
    def _set_den_bounds(self):
        ...
        cdef Integer N, p, delta, t0
        cdef Rational q_plus, q_minus, s
    ...
    def _initialise_an_coefficients(self):
        r"""
        Computes the Fourier coefficients `a_n` for all `n` up to 1000.
        Doctest in _test_init
    ...
    def _add_an_coefficients(self, int T):
        r"""
        Computes the Fourier coefficients `a_n` for all `n` up to `T`.
...
#================== Low level summation =========
    ...
    def _integration_to_tau(self, ComplexNumber tau,
        ...
        EXAMPLES::
            ...
            sage: M = ModularSymbolNumerical(EllipticCurve("11a1"))
            sage: M._integration_to_tau(0.01*I, 1000, 53)  # abs tol 1e-8
            0.253841860855911
            ...
            sage: ms._integration_to_tau(0.0001*I, 1000, 53) # abs tol 1e-8
            -0.0105693920159096
...
#================
    ...
    def _get_truncation_and_prec(self, double y, double eps):
        r"""
        Computes the numbers of terms needed in the sum approximating
        ...
        If `T` would be larger than `2^31`, the value (-1,-1) is returned instead.
...
# ===============
    ...
    cdef _from_r_to_rr_approx_direct(self, Rational r, Rational rr,
        ...
        a = rc._a
        Q = rc._width
        rrc = CuspsForModularSymbolNumerical(rr,self._N)
        mm = rrc._m
        aa = rrc._a
        QQ = rrc._width
        oi = llxgcd(Q*a, m, &u, &v)
    ...
    def _from_r_to_rr_approx(self, Rational r, Rational rr, double eps,
        ...
        EXAMPLES::
            ...
            sage: M = ModularSymbolNumerical(EllipticCurve("5077a1"))
            sage: M._from_r_to_rr_approx( 0/1, -7/31798, 0.001) # abs tol 1e-5
        ...
        """
        cdef llong m, Q, mm, QQ, a, aa
        cdef llong * wQ = [0L, 0L, 0L, 0L]
        cdef llong * wQQ = [0L, 0L, 0L, 0L]
        cdef Integer epsQ, epsQQ
        cdef Rational csq, x
        cdef double s, yy
        ...
        if method != "direct":
            ...
            yy = sqrt(yy)
            yy = 1/yy/m
            T1, _ = self._get_truncation_and_prec(yy, eps/4)
            yy = <double>QQ
            yy = sqrt(yy)
            yy = 1/yy/mm
    ...
    def _transportable_approx(self, Rational r, Rational rr, double eps):
        ...
        While this one goes via 0 ::
            ...
            sage: M._transportable_approx( 0/1, -7/31798, 0.001)# abs tol 1e-5
            -7.01577418382726e-9 - 7.40274138232394*I
            sage: M._from_r_to_rr_approx( 0/1, -7/31798, 0.001) # abs tol 1e-5
            -7.02253033502132e-9 - 7.40274138234031*I
        """
        cdef Integer c, a, b, d
        cdef int T, prec
        cdef double yy
...
#======= precise rationals =====
    ...
    def symbol_ioo_to_r(self, Rational r, int sign = 0,
        ...
        EXAMPLES::
            ...
            sage: from sage.schemes.elliptic_curves.mod_sym import ModularSymbolNumerical
            sage: M = ModularSymbolNumerical(EllipticCurve("11a1"))
            sage: M.symbol_ioo_to_r(0/1)
            1/5
            sage: M.symbol_ioo_to_r(3/11,-1)
            1/2

            sage: M = ModularSymbolNumerical(EllipticCurve("5077a1"))
            sage: M.symbol_ioo_to_r(0/1)
            0
            sage: M.symbol_ioo_to_r(123/456)
            -3
    ...
    def symbol_r_to_rr(self, Rational r, Rational rr, int sign = 0,
        ...
        computation may to choose to work directly along the path from
        ...
        EXAMPLES::
            ...
            sage: from sage.schemes.elliptic_curves.mod_sym import ModularSymbolNumerical
            sage: M = ModularSymbolNumerical(EllipticCurve("57a1"))
            sage: M.symbol_r_to_rr(0/1,8/9)
            0
            sage: M.symbol_r_to_rr(0/1,8/9,use_partials=False)
            0
            sage: M.symbol_r_to_rr(17/19,-5/9)
            1/2
            sage: M.symbol_r_to_rr(14/19,-5/9,-1)
            1/2
            sage: M.symbol_r_to_rr(174/179,-53/91,-1)
            -1
    ...
    def symbol_transportable(self, Rational r, Rational rr, int sign = 0):
        ...
        EXAMPLES::
            ...
            sage: from sage.schemes.elliptic_curves.mod_sym import ModularSymbolNumerical
            sage: M = ModularSymbolNumerical(EllipticCurve("11a1"))
            sage: M.symbol_transportable(0/1,-2/7)
            -1/2
            sage: M = ModularSymbolNumerical(EllipticCurve("37a1"))
            sage: M.symbol_transportable(0/1,-1/19)
            0
            sage: M.symbol_transportable(0/1,-1/19,-1)
            0
            sage: M = ModularSymbolNumerical(EllipticCurve("5077a1"))
            sage: M.symbol_transportable(0/1,-35/144)
            -3
            sage: M.symbol_transportable(0/1,-35/144,-1)
            0
            sage: M.symbol_transportable(0/1, -7/31798)
            0
    ...
    def manin_symbol(self, llong u, llong v, int sign = 0):
        ...
        EXAMPLES::
            ...
            sage: ms = ModularSymbolNumerical(E)
            sage: ms.manin_symbol(1,3)
            -1/2
            sage: ms.manin_symbol(1,3, sign=-1)
            -1/2
            sage: ms.manin_symbol(1,5)
            1
            sage: ms.manin_symbol(1,5)
            1
            ...
            sage: ms = ModularSymbolNumerical(E)
            sage: ms.manin_symbol(1,2)
            -1/2
            sage: ms.manin_symbol(17,6)
            -1/2
...
# ===============================
    ...
    def all_symbols(self, llong m, int sign=0):
        ...
        EXAMPLES::
            ...
            sage: m = ModularSymbolNumerical(E)
            sage: m.all_symbols(4)
            {1/4: 0, 3/4: 0}
            sage: m.all_symbols(8)
            {1/8: 1/2, 3/8: -1/2, 5/8: -1/2, 7/8: 1/2}
        """
        cdef llong N, Q, z, a, astar, j, epsQ
        ...
        res = dict()
        a = 1
        twopim = <double>6.28318530717958648
        twopim = twopim/m
        if sign == 1:
            while a < m:
                if llgcd(a,m) == 1:
                    ...
                    resam = <double>0
                    while j < m:
                        resam +=  v[j] * ( cos( twopim *j *a) - epsQ * cos(twopim*j*astar) )
                        j += 1
                    val = RR(resam)
            ...
            while a < m:
                if llgcd(a,m) == 1:
                    ...
                    resam = <double>0
                    while j < m:
                        resam +=  v[j] * ( sin( twopim *j *a) + epsQ *sin(twopim*j*astar) )
                        j += 1
                    val = RR(resam)
                    res[Rational( (a,m) )] = self._round(val, -1, True)
                a += 1
    ...
    def _twisted_symbol(self, Rational ra, int sign=0):
        ...
        EXAMPLE::
            ...
            sage: E = EllipticCurve("63a1")
            sage: from sage.schemes.elliptic_curves.mod_sym import ModularSymbolNumerical
            sage: M = ModularSymbolNumerical(E)
            sage: M(1/7, use_twist=True) # indirect doctest
            0
            sage: M(1/7, use_twist=False)
            0
        """
        cdef Integer D, Da, a
        cdef Rational res, t
...
def _test_init(E):
    ...
    - five integers representing the Fourier coefficients `a_1`, `a_2`, `a_3`, `a_389` and `a_2013`. (This will test _add_an_coefficients).

    - four integers that are bounds for the denominators as in _set_den_bounds
    ...
    EXAMPLES::
        ...
        sage: from sage.schemes.elliptic_curves.mod_sym import _test_init
        sage: _test_init(EllipticCurve("11a1")) # abs tol 1e-8
        ({1: 1, 11: -1}, [1, -2, -1, -15, -12], [10, 2, 10, 2],
        [0.06346046521397766,
        0.7294083084692475,
        0.06346046521397766,
        0.7294083084692475])
        sage: _test_init(EllipticCurve("11a2")) # abs tol 1e-8
        ({1: 1, 11: -1}, [1, -2, -1, -15, -12], [2, 2, 2, 2],
        [0.06346046521397766,
        0.7294083084692475,
        0.06346046521397766,
        0.7294083084692475])
        sage: _test_init(EllipticCurve("11a3")) # abs tol 1e-8
        ...
        0.11768229758162974,
        0.5685412997602702])
        sage: _test_init(EllipticCurve("37a1")) # abs tol 1e-8
...
def _test_integration(E, a, b, T):
    ...
    EXAMPLES::
        ...
        sage: from sage.schemes.elliptic_curves.mod_sym import _test_integration
        sage: E = EllipticCurve("11a1")
        sage: _test_integration(E, 0,0.01,1000) # abs tol 1e-5
        (0.2538418608559108+0j)
        sage: _test_integration(E, 0,0.0001,10000) # abs tol 1e-7
        (0.2538815728257322+0j)

        sage: E = EllipticCurve("37a1")
        sage: _test_integration(E, 0, 0.0001,1000) # abs tol 1e-8
        (-0.0105693920159094+0j)
        sage: _test_integration(E, 0.7, 0.1, 10000) # abs tol 1e-5
        (-0.021614803690068213-0.7770316490609953j)
        sage: _test_integration(E, 0.7, 0.1, 20000) # abs tol 1e-5
        (-0.021614803690068213-0.7770316490609953j)
    """
    M = ModularSymbolNumerical(E)
    c = complex(a,b)
...
def _test_integration_via_partials(E, y, m, T):
    ...
    EXAMPLES::
        ...
        sage: from sage.schemes.elliptic_curves.mod_sym import _test_integration_via_partials
        sage: E = EllipticCurve("11a1")
        sage: _test_integration_via_partials(E,0.001,3,1000) # abs tol 1e-8
        [-0.16916415619939476, 1.053687202321419, -0.6306661264594561]

        sage: E = EllipticCurve("121c1")
        sage: _test_integration_via_partials(E,0.03,3,700) # abs tol 1e-8
        [0.49198993741342745, 0.6601504274130794, 0.31770427139263896]
        sage: _test_integration_via_partials(E,0.03,3,7000)  # abs tol 1e-8
        [0.49198993741342745, 0.6601504274130794, 0.31770427139263896]
    """
...
def _test_against_table(range_of_conductors, verb=False):
    r"""
    ...
    EXAMPLE::
        ...
        sage: from sage.schemes.elliptic_curves.mod_sym import _test_against_table
        sage: _test_against_table([11,14]) # very long time
        True
    """
    boo = True
    ...
    for C in cremona_curves(range_of_conductors):
        ...
        li = [Rational((0,1)),Rational((1,1)),Rational((1,2)),
        ...
        for r in li:
            ...
            M2r = M(r, sign=-1)
            if verb:
                print "r=%s : (%s,%s),(%s, %s)    "%(r,mr,m2r,Mr,M2r),
            if mr != Mr or m2r != M2r:
                print "B u g : curve = %s, cusp = %s, sage's symbols (%s,%s), our symbols (%s, %s)"%(C.label(), r, mr, m2r, Mr, M2r)
                boo = False
    return boo
