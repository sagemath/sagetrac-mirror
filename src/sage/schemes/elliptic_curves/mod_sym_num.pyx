...
cdef class _CuspsForModularSymbolNumerical:
    ...
    """
    cdef public llong _a, _m, _width, _N4711
...
cdef class ModularSymbolNumerical:
    ...
    """
    cdef:
        llong _N4711,  _cut_val, _t_plus, _t_minus
...
# == initialisation ========
    ...
    def _set_epsQs(self):
        ...
        self._epsQs = dict(
            ...
            for d in Integer( self._N4711 ).divisors())
...
# ===============
    ...
    def _kappa(self, llong m, llong z, eps=None):
        ...
        precision::
            ...
            sage: M._kappa(7,9,0.1) # abs tol 1e-5
            [-3.848348562241613e-46,
            0.12314471107014528,
            -0.01516461914094593,
            -0.0012449611795634324,
    ...
    def _from_ioo_to_r_approx(self, Rational r, double eps,
        ...
        EXAMPLES::
            ...
            sage: E = EllipticCurve("5077a1")
            sage: m = ModularSymbolNumerical(E)
            sage: m._from_ioo_to_r_approx(-1/7,0.01, use_partials=1)  # abs tol 1e-5
            6.22747859174503 - 1.48055642530800*I
            sage: m._from_ioo_to_r_approx(-1/7,0.01, use_partials=0) #abs tol 1e-5
        ...
        rc = _CuspsForModularSymbolNumerical(r, self._N4711)
        Q = rc._width
        ...
        if m == 1:
            use_partials = 0
        if use_partials == 2:
            use_partials = (prec==53) and ( m**4 < self._N4711 or m < PARTIAL_LIMIT)
    ...
    cdef _from_r_to_rr_approx_direct(self, Rational r, Rational rr,
        ...
        EXAMPLES::
            ...
            ....: import ModularSymbolNumerical
            sage: M = ModularSymbolNumerical(EllipticCurve("11a1"))
            sage: M._from_r_to_rr_approx(1/3,0/1,0.000001,use_partials=0) # abs tol 1e-8
            -0.634604652139777 + 1.45881661693850*I
            sage: M._from_r_to_rr_approx(1/3,0/1,0.000001,use_partials=1) # abs tol 1e-8
        ...
        a = rc._a
        Q = rc._width
        rrc = _CuspsForModularSymbolNumerical(rr,self._N4711)
        mm = rrc._m
        aa = rrc._a
        QQ = rrc._width
        oi = llxgcd(Q*a, m, &u, &v)
        ...
        if use_partials == 2:
            ...
            D /= g
            D *=  llabs(a*mm-aa*m)
            use_partials = (prec==53) and ( D**4 < self._N4711 or D < PARTIAL_LIMIT)
    ...
    def _from_r_to_rr_approx(self, Rational r, Rational rr, double eps,
        ...
        EXAMPLES::
            ...
            sage: M._from_r_to_rr_approx(0/1,2/5,0.001, "both", use_partials=0) #abs tol 1e-5
            1.90381395641931 - 1.45881661693851*I
            sage: M._from_r_to_rr_approx(0/1,2/5,0.001, "both", use_partials=1) #abs tol 1e-5
            1.90381395641933 - 1.45881661693850*I
    ...
    def _transportable_approx(self, Rational r, Rational rr, double eps):
        ...
        from sage.modular.cusps import Cusp
        rc = Cusp(r)
        boo, ga = rc.is_gamma0_equiv(rr, self._N4711, "matrix")

        if not boo:
            raise ValueError("The cusps %s and %s are not "
                             "Gamma_0(%s)-equivalent"%(r, rr, self._N4711))

        # now find the same for the move to 0
        c = ga[1][0]
        r0 = - Rational( (c/self._N4711, ga[0][0]) )
        rc0 = Cusp(r0)
        _, ga0 = rc0.is_gamma0_equiv(0, self._N4711, "matrix")

        if c.abs() > ga0[1][0].abs(): # better at 0
            ga = ga0
            c = ga[1][0]
            eN = -self._epsQs[self._N4711]
        else: #better at i oo
            eN = 1
...
#======= precise rationals =====
    ...
    def _symbol_non_unitary(self, Rational r, int sign=0):
        ...
        cdef:
            llong a, m, B, Q, N_ell, aell, u, N = self._N4711
            Integer ell
            Rational r2, res
    ...
    def _manin_symbol_with_cache(self, llong u, llong v, int sign):
        ...
        """
        cdef:
            llong c, d, x, y, N = self._N4711, Mu, Mv, Qu, Qv, du=1, dv=1
            Rational r, rr, res
            int oi
        ...
        if u == 0:
            ...
            if isunitary: # unitary case
                oi = best_proj_point(u, v, self._N4711, &c, &d)
    ...
    def manin_symbol(self, llong u, llong v, int sign = 0):
        ...
        oi = proj_normalise(self._N4711, u, v, &un, &vn)
        ...
        # (-v:u) = - (u:v)
        oi = proj_normalise(self._N4711, -v, u, &un, &vn)
        ...
        oi = proj_normalise(self._N4711, u+v, -u, &un, &vn)
        if c.is_in_cache(un,vn,sign):
            r2 = - res - c(un,vn,sign)

            # (v:-u-v) = r2
            oi = proj_normalise(self._N4711, v, -u-v, &un, &vn)
            c.set_cache(r2, un, vn, sign)

            # (u+v:v) = -r2
            oi = proj_normalise(self._N4711, u+v, v, &un, &vn)
            c.set_cache(-r2, un, vn, sign)
        ...
        if c.is_in_cache(un,vn,sign):
            ...
            # (u:u+v) = -r2
            oi = proj_normalise(self._N4711, u, u+v, &un, &vn)
            c.set_cache(-r2, un, vn, sign)
...
#====================== approximative versions
    ...
    def _symbol_non_unitary_approx(self, Rational r, double eps):
        ...
        EXAMPLES::
            ...
            ....: import ModularSymbolNumerical
            sage: M = ModularSymbolNumerical(EllipticCurve("20a1"))
            sage: M._symbol_non_unitary_approx(1/2,0.0001) # abs tol 1e-6
        ...
        A bit longer take::
            ...
            sage: M = ModularSymbolNumerical(EllipticCurve("78a1"))
            sage: M._symbol_non_unitary_approx(1/38,0.1)  # abs tol 1e-4
            2.90087559068021 + 2.86538550720028e-7*I
            sage: M._symbol_non_unitary_approx(5/38,0.1)  # abs tol 1e-4
