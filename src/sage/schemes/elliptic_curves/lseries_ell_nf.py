...
EXAMPLES::
    ...
    sage: from sage.schemes.elliptic_curves.lseries_ell_nf import lseries_dokchitser
...
from sage.all import (PowerSeriesRing, Integer, QQ, ZZ,
                      Dokchitser, prime_range, prod)
from sage.categories.number_fields import NumberFields

from sage.lfunctions.helper_lseries import extend_multiplicatively_generic
...
#         sage: from sage.schemes.elliptic_curves.lseries_ell_nf import anlist_over_sqrt5
#         sage: K.<a> = NumberField(x^2-x-1); E = EllipticCurve([0,-a,a,0,0])
...
#     This function is not super fast, but at least it will work in a few
#     seconds up to `10^4`::
...
#     primes = K.primes_of_bounded_norm(bound + 1)
...
#     T = P.gen()
#     # Table of powers of T, so we do not have to compute T^4 (say) thousands of times.
...
def get_factor_over_nf(curve, prime_ideal, prime_number, conductor, accuracy):
    """
    Return the inverse of the factor corresponding to the given prime
    ideal in the Euler product expansion of the L-function at
    ``prime_ideal``.

    Unless the accuracy does not need this expansion, and
...
def get_coeffs_p_over_nf(curve, prime_number, accuracy=20, conductor=None):
    """
    Compute the inverse of product of L_prime on all primes above prime_number,
    then return power series of L_p up to desired accuracy.
    ...
    series_p = [get_factor_over_nf(curve, prime_id, prime_number,
                                   conductor, accuracy)
...
def anlist_over_nf(E, bound):
    ...
    EXAMPLES::
        ...
        sage: from sage.schemes.elliptic_curves.lseries_ell_nf import anlist_over_nf
...
def anlist(E, bound):
    r"""
    Compute the Dirichlet L-series coefficients, up to and including ``bound``.

    The i-th element of the return list is a[i].
    ...
    EXAMPLES::
        ...
        sage: from sage.schemes.elliptic_curves.lseries_ell_nf import anlist
    ...
    if E.base_field() == QQ:
        ...
        # should use anlist_over_sqrt5(E, bound)
    else:
...
def lseries_dokchitser(E, prec=53):
    ...
    Also ``prec`` is the number of bits of precision to
    ...
    Higher precision::
        ...
        sage: L = lseries_dokchitser(E, 200)  # long time
        sage: L(1)  # long time
    ...
    You can use this function as an algorithm to compute the sign of
    the functional equation (global root number)::
    ...
    Over QQ, this is not so useful (since Sage has a root_number
    method), but over number fields it is very useful::

        sage: K.<a> = NumberField(x^2 - x - 1)
        sage: E1 = EllipticCurve([0,-a-1,1,a,0])
        sage: lseries_dokchitser(E1, 16).eps
        -1
        sage: E1.rank()
        1
        sage: E0 = EllipticCurve([0,-a,a,0,0])
    ...
    # Check that we are over a number field.
    K = E.base_field()
    if K not in NumberFields():
        raise TypeError("base field must be a number field")

    # Compute norm of the conductor
    N = E.conductor()
    if N not in QQ:  # missing .norm() for elements of ZZ
        N = N.norm()
    ...
    if abs(L.check_functional_equation()) > tiny:
        ...
        # satisfied.  If it is, then we've got it.  If it is not, then
