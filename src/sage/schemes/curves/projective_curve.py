...
Closed points of the curve of arbitrary degree can be computed::
...
All singular closed points of the curve can be found::

    sage: C.singular_closed_points()
    [Point (x, y)]
    sage: p = _[0]
    sage: p.places()  # p is a unibranch singularity, that is a cusp.
    [Place (1/y)]
    sage: pls = _[0]
    sage: C.place_to_closed_point(pls)
    Point (x, y)

Transition to and from the function field of the curve is smooth::
...
class IntegralProjectiveCurve_finite_field(IntegralProjectiveCurve):
    ...
    def __init__(self, A, f):
        ...
        TESTS::
            ...
            True
        """
        super(IntegralProjectiveCurve_finite_field, self).__init__(A, f)
    ...
    def _singularities(self):
        """
        Return the list of the pairs of a singular closed point and the places above it.
        ...
        # and then collect places lying above each closed points
        points_and_places = []
        for place in places:
    ...
    def place_to_closed_point(self, place):
        """
        Return the closed point lying at the place.
