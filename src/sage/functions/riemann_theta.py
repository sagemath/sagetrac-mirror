...
def RiemannTheta_Constructor(Omega, deriv=[], uniform=True, deriv_accuracy_radius=5):
    r"""
    ...
    class RiemannTheta(Function_RiemannTheta):
        ...
        As shown in [CRTF], `n` th order derivatives introduce polynomial 
        ...
        def __init__(self, name, nargs, latex_name, conversions):
            ...
            Function_RiemannTheta.__init__(self, name, nargs, latex_name, conversions)
...
class Function_RiemannTheta(BuiltinFunction):
    ...
    def _plot_bounding_ellipsoid(self, z, R):
        ...
        if self._g == 2:
            ...
            P = implicit_plot(f,(xx,-R,R),(yy,-R,R))
        elif self._g == 3:
            # 3-D case
            var('xx,yy,zz')
            v = vector(SR,[xx,yy,zz])
            f = self._ring.pi()*(v+c).dot_product(self._Y*(v+c)) - R**2
            P = implicit_plot(f,(xx,-R,R),(yy,-R,R),(zz,-R,R))
        else: 
            # N-D case
    ...
    def integer_points(self, z, R):
        ...
        def find_integer_points(g, c, R, start):
            ...
            INPUT:
                ...
                - ``g`` -- the genus. recursively used to determine integer poiints along each axis.

                - ``c`` -- center of integer point computation. `0 \in \CC^g` is used when using the uniform approximation.

                - ``R`` -- the radius of the ellipsoid along the current axis.

                - ``start`` -- the starting integer point for each recursion along each axis.

            OUTPUT:

                - ``intpoints`` -- (list) a list of all of the integer points inside the bounding ellipsoid
    ...
    def _eval_(self, *args, **kwds):
        ...
        for arg in args:
            try:              self._ring(arg)
            except TypeError: has_symbolic = True

        if has_symbolic: return None
        else:            return self.value_at_point(*(args,), **kwds)        
    ...
    def _derivative_(self, *args, **kwds):
        ...
        return RiemannTheta_Constructor(self.Omega, newderiv, uniform=self.uniform, deriv_accuracy_radius=self.deriv_accuracy_radius)(*(args,), **kwds)
