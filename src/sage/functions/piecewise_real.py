r"""
Piecewise-defined Functions over the Reals

This module implement piecewise functions on the real line in a single variable.
See :mod:`sage.sets.real_set` for more information about how to construct
subsets of the real line for the domains. 

EXAMPLES::

    sage: f = piecewise_real([((0,1), x^3), ([-1,0], -x^2)]);  f
    piecewise_real(x|-->x^3 on (0, 1), x|-->-x^2 on [-1, 0]; x)
    sage: 2*f
    2*piecewise_real(x|-->x^3 on (0, 1), x|-->-x^2 on [-1, 0]; x)
    sage: f(x=1/2)
    1/8
...
class PiecewiseRealFunction(BuiltinFunction):
    ...
    def __init__(self):
        ...
        EXAMPLES::
            ...
            sage: var('x, y')
            (x, y)
            sage: f = piecewise_real([((0,1), x^2*y), ([-1,0], -x*y^2)], var=x);  f
            piecewise_real(x|-->x^2*y on (0, 1), x|-->-x*y^2 on [-1, 0]; x)
            sage: f(1/2)
            1/4*y
            sage: f(-1/2)
            1/2*y^2
        """
        BuiltinFunction.__init__(self, "piecewise_real", 
                                 latex_name="piecewise_real",
    ...
    def __call__(self, function_pieces, **kwds):
        ...
        EXAMPLES::
            ...
            sage: my_abs = piecewise_real([((-1, 0), -x), ([0, 1], x)], var=x);  my_abs
            piecewise_real(x|-->-x on (-1, 0), x|-->x on [0, 1]; x)
    ...
    def _subs_(self, subs_map, options, parameters, x):
        ...
        on::
            ...
            sage: p = piecewise_real([((-2, 0), -x), ([0, 2], x)], var=x)
            sage: p.subs(x=-1)
            1
            sage: (10+p).subs(x=-1)
            11
        ...
        Auxiliary variables can be substituted arbitrarily::
            ...
            sage: var('x,y')
            (x, y)
            sage: p = piecewise_real([((-2, 0), -x^y), ([0, 2], x-y)], var=x);  p
            piecewise_real(x|-->-x^y on (-2, 0), x|-->x - y on [0, 2]; x)
            sage: p.subs(y=sin(y))
        ...
        if point == x:
            ...
            return piecewise_real(new_params, var=x)
    ...
    def in_operands(ex):
        ...
        EXAMPLES::
            ...
            sage: f = piecewise_real([([0,0], sin(x)), ((0,2), cos(x))]);  f
            piecewise_real(x|-->sin(x) on {0}, x|-->cos(x) on (0, 2); x)
            sage: piecewise_real.in_operands(f)
            True
            sage: piecewise_real.in_operands(1+sin(f))
            True
            sage: piecewise_real.in_operands(1+sin(0*f))
            False
        """
        def is_piecewise_real(ex):
            result = ex.operator() is piecewise_real
            for op in ex.operands():
                result = result or is_piecewise_real(op)
            return result
        return is_piecewise_real(ex)
    ...
    class EvaluationMethods:
        ...
        def _fast_float_(cls, self, *args):
            ...
            EXAMPLES::
                ...
                sage: f = piecewise_real([([0,0], sin(x)), ((0,2), cos(x))])
                sage: f._fast_float_()
                Traceback (most recent call last):
                ...
                NotImplementedError
        ...
        def _fast_callable_(cls, self, parameters, variable, etb):
            ...
            EXAMPLES::
                ...
                sage: p = piecewise_real([((-1, 0), -x), ([0, 1], x)], var=x)
        ...
        def restriction(cls, self, parameters, variable, restricted_domain):
            ...
            EXAMPLES::
                ...
                sage: f = piecewise_real([((-oo, oo), x)]);  f
                piecewise_real(x|-->x on (-oo, +oo); x)
                sage: f.restriction([[-1,1], [3,3]])
                piecewise_real(x|-->x on [-1, 1] + {3}; x)
            """
            ...
            return piecewise_real(new_param, var=variable)
        ...
        def extension(cls, self, parameters, variable, extension, extension_domain=None):
            ...
            EXAMPLES::
                ...
                sage: f = piecewise_real([((-1,1), x)]);  f
                piecewise_real(x|-->x on (-1, 1); x)
                sage: f(3)
                Traceback (most recent call last):
                ...
                ValueError: point is not in the domain
                ...
                piecewise_real(x|-->x on (-1, 1), x|-->0 on (-oo, -1] + [1, +oo); x)
                sage: g(3)
                0

                sage: h = f.extension(1, RealSet.unbounded_above_closed(1));  h
                piecewise_real(x|-->x on (-1, 1), x|-->1 on [1, +oo); x)
                sage: h(3)
                1
            """
            self = piecewise_real(parameters, var=variable)
            ...
            return piecewise_real(parameters + ext, var=variable)
        ...
        def pieces(cls, self, parameters, variable):
            ...
            EXAMPLES::
                ...
                sage: p = piecewise_real([((-1, 0), -x), ([0, 1], x)], var=x)
                sage: p.pieces()
                (piecewise_real(x|-->-x on (-1, 0); x), 
                 piecewise_real(x|-->x on [0, 1]; x))
            """
            result = []
            for domain, func in parameters:
                result.append(piecewise_real([(domain, func)], var=variable))
            return tuple(result)




piecewise_real = PiecewiseRealFunction()
