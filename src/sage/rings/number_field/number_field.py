...
class NumberField_generic(number_field_base.NumberField):
    ...
    def _S_class_group_and_units(self, S, proof=True):
        ...
        - ``clgp_gens`` - A list of tuples ``(gen, order, pr)``, where ``gen``
          is a fractional ideal representative of the `S`-class group generator
          whose order (in the `S`-class group) is ``order``, and ``pr`` is a
          principal generator of `gen^{order}` if the latter is indeed
          principal; in general, there is some principal ideal in the coset of
          `gen^{order}` with respect to the subgroup of the class group
          generated by ideals in `S` and ``pr`` is a principal generator of
          this ideal.
        ...
        Verify that :trac:`14489` is fixed::
            ...
            ([-7/13*a^2 - 140/13*a + 36/13,
             ...
             [(Fractional ideal (11, a - 2), 2, -2/13*a^2 - 53/13*a + 92/13),
        ...
        for k in range(len(pari_cyc)):
            ...
            if order > 1: # since some generators are given "of order 1"
                ...
                if card_S != 0 and not J.is_principal():
                    B = H(J).exponents()
                    pari_B = (-pari(B)).Col()
                    exps = pari(MS).matsolvemod(pari_ords, pari_B).Vec().sage()
                    Spart = prod([S[i] ** (exps[i] % Sords[i]) for i in range(card_S)])
                    J *= Spart
                clgp_gens.append(   (I, order, self(J.gens_reduced()[0]))   )
