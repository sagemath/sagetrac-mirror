...
class FunctionField(Field):
    ...
    @cached_method
    def jacobian(self, model=None, base_div=None):
        """
        ...
        if model.startswith('km'):
            ...
            if model == 'km' or model.endswith('large'):
                ...
                return Jacobian(self, base_div, model='large')
                ...
                if not base_div.degree() >= 2*g + 1:
                    raise ValueError("Khuri-Makdisi medium model requires base divisor of degree "
                                     "at least 2*g + 1 for genus g")
                return Jacobian(self, base_div, model='medium')
                ...
                return Jacobian(self, base_div, model='small')
        elif model == 'hess':
