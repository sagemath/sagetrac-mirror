...
cdef class MPolynomial(CommutativeRingElement):
    ...
    def zeta_function(self, p=None, N=None, affine=False, verbose=False):
        ...
        if verbose:
            ...
            # Access to stdout to flush prints
        ...
        # 0-dimensional polytope (f = ax^u, a,u != 0), return 1
            ...
            if affine:
                raise NotImplementedError(
                    "Computing affine zeta functions of a"+
        ...
        with localvars(self.parent(), ["x%d" % i for i in range(1,n+1)]):
            self = Rx(self)
        ...
        for d in range(1,n+1):
            ...
            J[d] = J[d].stack(
                identity_matrix(len(RwD[d][0]))[nonpivots, :]
            )
        ...
        ell = map(lambda a: _ThetaCoefficient(a, p, N), ell)
        ...
        for vi in V:
            ...
            for t in ker:
                ...
                outer = [floor((N+1)*ZZ(p)/ZZ(p-1) - ki/ZZ(p)) for ki in k]

                piece2 = 0
                for e in IntegerListsLex(length=U.nrows(), min_sum=0, max_sum=floor(limit), ceiling=outer):
        ...
        for G in alpha_V:
                ...
                if d > n+1:
                    for Ut, Uti in Uts:
                        ...
                        if all(yi >= 0 for yi in y):
                            for z in IntegerVectors(d-(n+1), n+1, outer=map(floor, y)):
                                z = vector(ZZ, z)
                                um0 = (y-z) * Ut
                                if affine:
            ...
            if G != 0:
                print("Something is wrong, didn't reduce completely")
...
class _ThetaCoefficient:
    ...
    def __mul__(self, other):
        ...
        if self.p != other.p:
            raise ValueError("Can't combine coefficients"
