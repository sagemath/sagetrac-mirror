...
def list_monom(P, d):
    ...
    The list of monomials in P of degree d, ordered from the biggest
    to the smallest according to the monomial ordering on P
...
def firstvar(mon):
    ...
    The index of the first variable appearing in the monomial mon, or
    -1 for a constant monomial
...
class MacaulayMatrix:
    ...
    def __init__(self, base, monomials, entries, labels=None, signatures=None,
                 s=None, indexes=None):
        r"""
    ...
    def signature_tropical(self,i,tropical_weight, tropical_order_ring):
        ...
        max_trop_weight = sum([listmonom[0].degrees()[v] * tropical_weight[v]
                               for v in range(nvar)])
        for j0 in range(1, l):
            term_trop_weight = sum([listmonom[j0].degrees()[w] *
                                    tropical_weight[w] for w in range(nvar)])
    ...
    def polynomial(self, i):
        ...
        return sum([row[j] * monomials[j]
                    for j in range(self._indexes[i], self._ncols)])
    ...
    def copy(self):
        ...
        """
        if self._labels is not None:
            labels = Matrix(self._base, self.nrows(), self._s,
                            self._labels.list())
        else:
            labels = None
        ...
        l = Matrix(self._base.base_ring(), self.nrows(), self.ncols(),
                   self._entries.list())
        monom = [x for x in self._monomials]
        signs = None
        if self._signatures is not None:
            signs = [y for y in self._signatures]
        return self.__class__(self._base, monom, l, labels, signs, self._s,
                              indexes)
    ...
    def addf(self, f, flab=None, fsign=None):
        ...
        .. NOTE::
            ...
            We assume that f has the correct degree and labels and
    ...
    def add_multiple_of_row(self, i, rpiv, kpiv, eliminate=False):
        ...
        Perform self.row(i)=self.row(i)+kpiv*self.row(rpiv),
        ...
        .. NOTE::
            ...
            When signatures are involved, we assume that this
    ...
    def column_transposition(self, j1, j2):
        """
        Perform the transposition of the columns j1 and j2 of the Macaulay
        matrix self.
        ...
        if j1 != j2:
            ...
            for i in range(nrows):
                if self._indexes[i] != ncols:
                    if max(j1, j2) >= self._indexes[i] >= min(j1, j2):
                        j = min(self._indexes[i], j1, j2)
                        while j < ncols and self._entries[i, j].is_zero():
                            j += 1
                        self._indexes[i] = j

    def row_echelon(self, pivot_choice=False, Macdd=None, l=None,
                    compute_pivots=False, tropical=False, tropical_weight=None,
                    tropical_order_ring=None, precMF5loc=None):
        """
    ...
    def update_mac(self, d, i, fi, di, Macdi, allmonomials, Macdd=None,
                   use_criteria=True, tropical=False, tropical_weight=None,
                   tropical_order_ring=None, grevlex=False):
        """
        INPUT:
                            ...
                            if not(Macdi.F5crit(sign[0] * x)):
                                if self._labels is not None:
                                    if self._signatures is not None:
                                        ...
                                        nsign[0] = x * nsign[0]
                                        self.addf(x * Macdd.polynomial(k),
                                                  [x * lab for lab in Macdd.label(k)], nsign)
                                    else:
                                        self.addf(x * Macdd.polynomial(k),
                                                  [x * lab for lab in Macdd.label(k)])
                ...
                for mono in listsign:
                    ...
                    if self._labels is not None:
                        if self._signatures is not None:
                            ...
                            nsign[0] = y * nsign[0]
                            self.addf(y * Macdd.polynomial(row),
                                      [y * lab for lab in Macdd.label(row)],
                                      nsign)
                        else:
                            self.addf(y * Macdd.polynomial(row),
                                      [y * lab for lab in Macdd.label(row)])
        ...
        if tropical:
            ...
            while g.degree() >= 0:
                ...
                if u in dict_monom:
                    ...
                    g = g - coef * self.polynomial(i)
                    if flab is not None:
                        rlab = [rlab[j] + coef * self._labels[j]
                                for j in range(s)]
                else:
                    r = r + g.lt()
                    g = g - g.lt()
...
def toy_matrixF5(F, D, P, label=True, fast=True, weak_filling=False,
                 matrices=False, use_criteria=True, minimal=True,
                 reduced=False, label_of_output=False, tropical=False,
                 tropical_weight=None, tropical_order_ring=None,
                 prec_theo=None):
    r"""
    INPUT:
    ...
    .. [Vac2015] \T. Vaccon. *Matrix-F5 Algorithms and Tropical Gröbner Bases Computation*, proceedings of ISSAC 2015.
    """
