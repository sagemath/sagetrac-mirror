...
cpdef create_RealFloatingPointField(int prec=53, int sci_not=0, rnd=MPFR_RNDN):
    """
...
cdef class RealFloatingPointField_class(sage.rings.ring.Field):
    ...
    def to_prec(self, prec):
        ...
        if prec == self.__prec:
            ...
            return create_RealFloatingPointField(prec, self.sci_not, self.rnd)
...
cdef class RealNumber(sage.structure.element.RingElement):
        ...
        if can_use_int_literal or can_use_float_literal:
            ...
            if rnd == MPFR_RNDN:
                ...
                if rnd == MPFR_RNDD or rnd == MPFR_RNDZ:
                    fld = create_RealFloatingPointField(self.prec() + 1, rnd='RNDU')
                else:
                    fld = create_RealFloatingPointField(self.prec() + 1, rnd='RNDD')
                s = fld(self).str()
    ...
    def __pari__(self):
        ...
        if rounded_prec > prec:
            self = create_RealFloatingPointField(rounded_prec)(self)
    ...
    def _mpmath_(self, prec=None, rounding=None):
        ...
        """
        if prec is not None:
            return create_RealFloatingPointField(prec)(self)._mpmath_()
    ...
    def simplest_rational(self):
        ...
        if rnd == MPFR_RNDN:
            # hp == "high precision"
            hp_field = create_RealFloatingPointField(prec + 1)
...
cdef class RealLiteral(RealNumber):
    ...
    def numerical_approx(self, prec=None, digits=None, algorithm=None):
        ...
        return create_RealFloatingPointField(prec)(self.literal)


RR = create_RealFloatingPointField()

RR_min_prec = create_RealFloatingPointField(MPFR_PREC_MIN)
...
def create_RealNumber(s, int base=10, int pad=0, rnd="RNDN", int min_prec=53):
    ...
    if base == 10 and min_prec == 53 and len(s) <= 15 and rnd == "RNDN":
        ...
        R = create_RealFloatingPointField(prec=max(bits+pad, min_prec), rnd=rnd)
...
def __create__RealFloatingPointField_version0(prec, sci_not, rnd):
    ...
    EXAMPLES::
        ...
        Real Floating Point Field with 53 bits of precision
    """
    return create_RealFloatingPointField(prec, sci_not, rnd)
