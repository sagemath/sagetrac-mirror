...
class AlgebraicField_common(sage.rings.ring.Field):
    ...
    def subfield(self, numbers, name='a'):
        """
        Computes the number field generated by the numbers passed.

        INPUT:

            - ``numbers`` -- an element of self, or a list or tuple of them.

            - ``name`` -- (default: 'a') a string with the name of the generator of the resulting field.

        OUTPUT:

            - a tuple, whose first entry is a number field F, and the second is a list with the elements passed expressed in F.
        ...
        EXAMPLES::
            ...
            sage: b
            1.442249570307409?
            sage: QQbar.subfield([a,b],'t')
            (Number Field in t with defining polynomial x^6 - 6*x^4 - 6*x^3 + 12*x^2 - 36*x + 1,
            [-48/755*t^5 - 27/755*t^4 + 64/151*t^3 + 468/755*t^2 - 879/755*t + 1092/755,
            -48/755*t^5 - 27/755*t^4 + 64/151*t^3 + 468/755*t^2 - 124/755*t + 1092/755])
            sage: [ap,bp]=QQbar.subfield([a,b],'t')[1]
            sage: QQbar(ap)
            1.414213562373095?
            sage: QQbar(bp)==b
            True
        ...
        if not isinstance(numbers,(list,tuple)):
            return self.subfield([numbers],name)
        elif len(numbers)==1:
            F=NumberField(numbers[0].minpoly(),name,embedding=numbers[0])
            return (F,[G.gen()])
        l=list(numbers)
        cuerpos=[NumberField(l[i].minpoly(),name+str(i),embedding=l[i]) for i in range(len(l))]
        F=cuerpos.pop(0)
        gens=[F.gen()]
        while len(cuerpos)>0:
            F2=cuerpos.pop(0)
            if F2.gen() in QQ:
                gens.append(F(QQ(F2.gen())))
            else:
                extension=F.composite_fields(F2,name,both_maps=True)
                h1=extension[0][1]
                h2=extension[0][2]
                gens=map(h1,gens)
                gens.append(h2(F2.gen()))
                F=extension[0][0]
        return (F,gens)
