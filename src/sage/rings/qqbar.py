...
from sage.structure.sage_object import (SageObject, py_rich_to_bool, op_EQ,
        op_NE, op_LE, op_LT, op_GE, op_GT)
...
class AlgebraicNumber(AlgebraicNumber_base):
    ...
    def _richcmp_(self, other, op):
        ...
        if type(sd) is ANRational and type(od) is ANRational:
            return py_rich_to_bool(op, cmp(sd._value, od._value))
        ...
        return py_rich_to_bool(op, self._cmp_(other))

    def _cmp_(self, other):
        r"""
        Comparisons for algebraic numbers.
        ...
        if not ri1.overlaps(ri2):
            return ri1._cmp_(ri2)

        # case 1: rationals
        sd = self._descr
        od = other._descr
        if type(sd) is ANRational and type(od) is ANRational:
            return cmp(sd._value, od._value)
...
class AlgebraicReal(AlgebraicNumber_base):
    ...
    def _cmp_(self, other):
        r"""
        Comparison for real algebraic numbers
        ...
        if not self._value.overlaps(other._value):
            return self._value._cmp_(other._value)
    ...
    def sign(self):
        ...
        sd = self._descr
        if type(sd) is ANRational:
            return sd._value.sign()
        elif type(sd) is ANExtensionElement:
