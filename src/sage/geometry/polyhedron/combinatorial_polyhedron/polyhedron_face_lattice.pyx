...
cdef class PolyhedronFaceLattice:
    ...
    cdef CombinatorialFace get_face(self, int dimension, size_t index):
        ...
        EXAMPLES::
            ...
            sage: it = C.face_iter(dimension=1)
            sage: face = next(it)
            sage: face_via_all_faces_from_iterator(it, C).Vrep()
            (A vertex at (3, 1, 4, 2), A vertex at (3, 2, 4, 1))
            sage: face.Vrep()
            (A vertex at (3, 1, 4, 2), A vertex at (3, 2, 4, 1))
            sage: all(face_via_all_faces_from_iterator(it, C).Vrep() ==
            ....:     face.Vrep() for face in it)
            True
            ...
            (A vertex at (-1/2, 1/2, -1/2, -1/2),
             A vertex at (-1/2, 1/2, 1/2, -1/2),
             A vertex at (0, 0, 0, -1))
            sage: all(face_via_all_faces_from_iterator(it, C).Vrep(False) ==
            ....:     face.Vrep(False) for face in it)
            True
        """
        cdef size_t length
        if self.dual:
