...
cdef class FaceIterator_base(SageObject):
    ...
    def _meet_of_coatoms(self, *indices):
        ...
        cdef face_length = self.structure.face_length
        ...
        cdef ListOfFaces face_mem = ListOfFaces(1, n_atoms)
        cdef uint64_t *face = face_mem.data[0]
        cdef size_t i
    ...
    def _join_of_atoms(self, *indices):
        ...
        cdef ListOfFaces face_mem = ListOfFaces(2, n_atoms)
        cdef uint64_t **faces = face_mem.data
        cdef uint64_t *face = faces[0]
        ...
        if not indices:
            ...
            for i in range(face_length):
                face[i] = 0
        elif not self._bounded and is_subset(face, self.structure.visited_all[0], face_length):
    ...
    cdef inline int next_face_loop(self) except -1:
        ...
        if newfacescounter:
            ...
            self.structure.current_dimension -= 1
            self.structure.first_time[self.structure.current_dimension] = True
            self.structure.n_newfaces[self.structure.current_dimension] = newfacescounter
            self.structure.n_visited_all[self.structure.current_dimension] = n_visited_all
            self.structure.yet_to_visit = newfacescounter
            return 0
        else:
    ...
    cdef int find_face(self, uint64_t *face) except -1:
        ...
        """
        cdef face_length = self.structure.face_length
        cdef size_t n_atoms = count_atoms(face, face_length)
        ...
        while self.structure.current_dimension != self.structure.dimension:
            if is_subset(face, self.structure.face, face_length):
                if is_subset(self.structure.face, face, face_length):
                    # Found our face.
                    return 0
            else:
