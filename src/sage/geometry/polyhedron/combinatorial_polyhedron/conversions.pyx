...
cdef int Vrep_list_to_bit_rep(tuple Vrep_list, face_struct& output) except -1:
    ...
    - ``vertex_list`` -- tuple of pairwise distinct positive integers
    - ``output`` -- a face that fits all integers in ``vertex_list``
...
cdef int incidences_to_bit_rep(tuple incidences, face_struct& output) except -1:
    ...
    - ``incidences`` -- tuple of integers representing incidences
      that fit into ``output``
    - ``output`` -- an initialized face

    OUTPUT:

    - ``output`` is filled with the incidences
...
def facets_tuple_to_bit_rep_of_Vrep(tuple facets_input, size_t n_Vrep):
    ...
    for input_facet in range(n_facets):
        for input_Vrep in facets_input[input_facet]:
            ...
            face_add_atom(Vrep.data.faces[input_Vrep], input_facet)
    return Vrep
...
cdef inline size_t bit_rep_to_Vrep_list(face_struct& face, size_t *output) except -1:
    r"""
    ...
    cdef size_t j = face_next_atom(face, 0)
    while (j != -1):
