...
- congruence test (i.e. does there exists n such that a conjugate of the group contains Gamma(n))
...
from hyperbolic_geometry import HH
...
def PSL_rep(m,n):
    ...
    if n%2:
        limit = Zmod(n)((n+1)//2)
        if a != 0:
            if a < limit: res=m_mod
            else: res=inv*m_mod

        else:
            if b < limit: res=m_mod
            else: res=inv*m_mod
...
def gamma0_triangle_graph(n,return_mapping=False):
    r"""
    Returns the coset graph for gamma0
    """
    T = matrix([[1,1],[0,1]])
    g2 = matrix([[0,-1],[1,0]])
    g2.set_immutable()
    ...
    for i in xrange(1,n): matrices[PSL_rep(T**i, n)] = 0
    ...
    while l:
        ...
        if not matrices.has_key(x1):
            for i in xrange(n): matrices[PSL_rep(T**i*x1,n)] = nb
            nb += 1
            edges[0].append(None)
            edges[1].append(None)
            l.add(x1)
