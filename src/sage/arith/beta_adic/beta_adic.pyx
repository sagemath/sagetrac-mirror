...
cdef initCdInfoBetaAdic(self, InfoBetaAdic *i, Cd, verb=False):
    ...
    i.nc = len(Cd)
    if i.nc > i.ncmax:
        raise ValueError("Trop de chiffres : %d > %d max (initialiser le BetaAdic avec plus de chiffre)."%(i.nc, i.ncmax))
...
#        print "C="; print self.C
    ...
    def _repr_(self):
        ...
        EXAMPLES::
            ...
            sage: BetaAdic((1+sqrt(5))/2, {0,1})
            Monoid of b-adic expansion with b root of x^2 - x - 1 and numerals set {0, 1}
            sage: BetaAdic(3, {0,1,3})
            Monoid of 3-adic expansion with numerals set {0, 1, 3}

        TESTS::

            sage: m=BetaAdic(3/sqrt(2), {0,1})
            sage: repr(m)
...
#         EXAMPLES::
# 
#             sage: m=BetaAdic((1+sqrt(5))/2, {0,1})
#             sage: m.default_ss()
...
#                c = kp.gen()
    ...
    def intersection(self, ss, ss2=None, Iss=None,
        ...
        if ss2 is None:
            ...
            if type(ss) == type(BetaAdic(2, {0, 1})):
                m = ss
                ss = m.ss
                if m.b != self.b:
...
#         return ad
    ...
    def Proj(self, a, b, t=0, arel=None):
        m2 = BetaAdic(self.b, set(a.alphabet+b.alphabet))
        if arel is None:
    ...
    def Approx_rec(self, DetAutomaton a, test, f, x, int n, int n2):
        ...
        EXAMPLE::
            ...
            sage: m = BetaAdic((x^3-x^2-x-1).roots(ring=QQbar)[1][0], {0,1})
            sage: pm = m.b.parent().places()[1]
    ...
    def compute_translations2(self, DetAutomaton aoc, imax=None, verb=False):
        ...
        # m2 = BetaAdic(b, set([t1 - t2 for t1 in A for t2 in A]))
