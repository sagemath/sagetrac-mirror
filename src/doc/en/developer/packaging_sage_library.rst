...
command::
  ...
  $ ./bootstrap && ./sage -sh -c '(cd pkgs/sagemath-standard && tox -v -v -v -e py39-sagewheels-nopypi)'
...
Again we can run the test with ``tox`` in a separate virtual environment::
  ...
  $ ./bootstrap && ./sage -sh -c '(cd pkgs/sagemath-standard-no-symbolics && tox -v -v -v -e py39-sagewheels-nopypi)'
...
without relying on the wheels from the Sage build::
  ...
  $ ./bootstrap && ./sage -sh -c '(cd pkgs/sagemath-objects && tox -v -v -v -e py39)'
