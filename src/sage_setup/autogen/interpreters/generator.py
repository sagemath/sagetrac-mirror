...
class InterpreterGenerator(object):
    ...
    def write_interpreter_h(self, write):
        r"""
        Generate the code for the C interpreter's associated header file.
        ...
        EXAMPLES::
            ...
            sage: gen.write_interpreter_h(buff.write)
            sage: print(buff.getvalue())
        ...
        w(je(ri(0, """
            /* {{ warn }} */
            /* This header is only for internal use when building the
               interpreters package */
            #ifndef _INTERP_{{ s.name.upper() }}_H
            ...
            #endif  /* _INTERP_{{ s.name.upper() }}_H */
            """), s=s, myself=self, warn=AUTOGEN_WARN))
    ...
    def write_interpreter(self, write):
        ...
        w(je(ri(0, """
            /* {{ warn }} */
            #include "interpreters/interp_{{ s.name }}.h"
    ...
    def write_wrapper(self, write):
        ...
        w(je(ri(0, """
            ...
            cdef extern from "interpreters/interp_{{ s.name }}.h":
    ...
    def get_interpreter_h(self):
        r"""
        Return the code for the C interpreter's header file.

        The use of this header file is not strictly needed on all platforms,
        as Cython will otherwise generate correct declarations for externally
        defined functions directly in the generated C source when using the
        ``cdef extern:`` construction.

        However, the declarations it produces are not correct on Windows due to
        incorrect use of the ``DL_IMPORT`` macro.  These headers are required
        to work around that.  See http://trac.sagemath.org/ticket/19868 for
        discussion.
        ...
        Next it includes ``<Python.h>`` in each header.  This is needed for
        most of the interpreter C sources in the first place.  But it's also
        where ``HAVE_DECLSPEC_DLL`` is defined for those platforms that use
        DLLs (basically any compiler on Windows/Cygwin).  It's also where the
        ``DL_IMPORT`` macro is defined on those platforms, but this macro has
        been deprecated in Python since v2.3 and is not used anywhere by Python,
        but it *is* used, incorrectly, by Cython.

        We redefined ``DL_IMPORT`` here to ``__declspec(dllexport)`` since this
        header is only included when building the interpreter modules themselves
        and are *not* meant for external code to include (the only case where
        ``__declspec(dllimport)`` should ever be used)::
        ...
        After this declaration there is a per-interpreter 'header' to this
        header file, where each interpreter may add includes or other
        declarations needed by the declaration for its ``interp_`` function
        (the only other thing in this file).  By default this per-interpreter
        header is empty::
        ...
        But the CDF interpreter, among others, customizes this by setting
        ``self.h_header`` in its ``__init__``::
        ...
        buff = StringIO()
        self.write_interpreter_h(buff.write)
