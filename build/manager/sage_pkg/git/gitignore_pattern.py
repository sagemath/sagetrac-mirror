...
From https://github.com/cpburnz/python-path-specification

LICENSE: 

* Original code is MPLv2
* We use the MPLv2 provision 3.3 to distribute it under the terms of GPLv2+

EXAMPLES::

    >>> from sage_pkg.git.gitignore_pattern import GitIgnorePattern
    >>> GitIgnorePattern('/foo/bar/baz.py', root='/usr/src')
    gitignore:^/usr/src/foo/bar/baz\.py$
    >>> GitIgnorePattern('/baz.py', root='/usr/src')
    gitignore:^/usr/src/baz\.py$
    >>> GitIgnorePattern('baz.py', root='/usr/src')
    gitignore:^/usr/src/(?:.+/)?baz\.py$
    >>> pattern = GitIgnorePattern('*.py', root='/usr/src');  pattern
    gitignore:^/usr/src/(?:.+/)?[^/]*\.py$
    >>> pattern.matches('/usr/src/foo/bar.py')
    True
"""
...
class GitIgnorePattern(object):
    """
    A gitignore pattern compiled to a regex.
    """

    def __init__(self, pattern, root=None):
        """
        Initializes the ``GitIgnorePattern`` instance.

        INPUT:

        - ``pattern`` -- string. The gitignore pattern.

        - ``root`` -- string. The root path for the gitignore leading
          slash pattern.
    ...
    def __repr__(self):
        return 'gitignore:' + str(self.regex.pattern)
