SAGE_SPKG_CONFIGURE([gfortran], [
    AC_REQUIRE([SAGE_SPKG_CONFIGURE_GCC])
      ...
      # /usr/bin/ls, we will at least know it is not a working Fortran
      # compiler.
    AS_IF([ test "$SAGE_HAVE_FC_FREEFORM" != yes ], [
      AC_MSG_NOTICE([Either there is no Fortran compiler installed, or the])
      AC_MSG_NOTICE([Fortran compiler does not accept free-format source code])
