...
SAGE_ROOT_LOGS ?= logs-$(shell cat src/VERSION.txt)

logs:
	@if [ -e logs -a ! -L logs ]; then mv -v logs logs-moved-away-$$(date +%Y-%m-%dT%H-%M-%S); fi
	@log_dir=$(SAGE_ROOT_LOGS) && mkdir -p $$log_dir/pkgs && rm -f logs && ln -sv $$log_dir logs
...
%::
	...
	$(MAKE) logs build/make/Makefile --stop
	+build/bin/sage-logger \
		"cd build/make && ./install '$@'" $(SAGE_ROOT_LOGS)/install.log
...
.PHONY: logs default build dist install micro_release \
